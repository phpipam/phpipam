...
if (sizeof($address) > 1) {
    ...
    print "	<td>";
    $Sections->print_breadcrumbs($Sections, $Subnets, ["page" => "subnets", "section" => $subnet['sectionId'], "subnetId" => $subnet['id'], "ipaddrid" => $address['id']], $Addresses);
    print "</td>";
    print "</tr>";
    ...
    } elseif ($address['state'] == "2") {
        $stateClass = _("Reserved");
    } elseif ($address['state'] == "3") {
        $stateClass = _("DHCP");
    } else {
        $stateClass = _("Online");
    }
    ...
    if ($address['excludePing'] == 1) {
        ...
        $seen_status = "success";
        $seen_text = _("Device is alive") . "<br>" . _("Last seen") . ": " . $address['lastSeen'];
    } elseif ($tDiff < $statuses[1]) {
        $seen_status = "warning";
        $seen_text = _("Device warning") . "<br>" . _("Last seen") . ": " . $address['lastSeen'];
    } elseif ($tDiff > $statuses[1]) {
        $seen_status = "error";
