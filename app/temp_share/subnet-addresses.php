...
<h4 style="margin-top:40px;">
    <?php
    if (!$slaves) {
        print _("IP addresses in subnet ");
    } else {
...
<table class="ipaddresses sortable sorted normalTable table table-condensed table-full table-top"
    ...
    <tr class="th">
        ...
        if (in_array('owner', $selected_ip_fields)) {
            print "<th class='hidden-xs hidden-sm hidden-md'>" . _('Owner') . "</th>";
        }
    ...
    if ($addresses === false || sizeof($addresses) == 0) {
        ...
        foreach ($addresses as $dummy) {
            ...
            if ($n == 0) {
                $unused = $Addresses->find_unused_addresses($Subnets->transform_to_decimal($subnet_detailed['network']), $addresses[$n]->ip_addr, $subnet['mask']);
            } # check unused space between IP addresses
            else {
                $unused = $Addresses->find_unused_addresses($addresses[$n - 1]->ip_addr, $addresses[$n]->ip_addr, $subnet['mask']);
            }
            ...
            if ($addresses[$n]->class == "range-dhcp") {
                ...
                if (in_array('state', $selected_ip_fields)) {
                    if ($addresses[$n]->state == 0) {
                        $stateClass = _("Offline");
                    } elseif ($addresses[$n]->state == 2) {
                        $stateClass = _("Reserved");
                    } elseif ($addresses[$n]->state == 3) {
                        $stateClass = _("DHCP");
                    }
                }
            ...
            ****************************************************/
            if ($n == $m) {
                ...
                if (isset($addresses[$n]->stopIP)) {
                    $unused = $Addresses->find_unused_addresses($addresses[$n]->stopIP, $Subnets->transform_to_decimal($subnet_detailed['broadcast']), $subnet['mask']);
                } else {
                    $unused = $Addresses->find_unused_addresses($addresses[$n]->ip_addr, $Subnets->transform_to_decimal($subnet_detailed['broadcast']), $subnet['mask']);
                }
