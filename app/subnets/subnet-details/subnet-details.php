...
<table class="ipaddress_subnet table-condensed table-full">
	...
	if($User->settings->enableVRF==1 && $User->get_module_permissions ("vrf")>0) {
		...
		if($vrf===false) {
			...
			# set text
			$vrfText = "<a href='/autodb/index.php?page=tools&section=vrf&subnetId=".$vrf->vrfId."' target='_blank'>".$vrf->name."</a>";
	...
	<tr>
		...
		if(!empty($subnet['device'])) {
			...
			if ($device!==false) {
				...
				print "<a href='/autodb/index.php?page=tools&section=devices&subnetId=".$device->id."'>".$device->hostname."</a>";
	...
	<tr>
		...
		if(!empty($subnet['location']) && $subnet['location']!=0) {
            ...
            if($location2!==false) {
                print "<a href='/autodb/index.php?page=tools&section=locations&subnetId=".$location2->id."'>$location2->name</a>";
            }
		}
