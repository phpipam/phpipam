...
if (sizeof($clogs) == 0) {
    ...
    foreach ($clogs as $l) {
        ...
        # folder?
        if ($subnet['isFolder'] == 1) {
            print "	<td><a href='" . create_link("subnets", $_GET['section'], $_GET['subnetId']) . "'>$subnet[description]</a></td>";
        } else {
            print "	<td><a href='" . create_link("subnets", $_GET['section'], $_GET['subnetId']) . "'>$subnet[ip]/$subnet[mask]</a></td>";
        }
...
if ($clogsSlaves) {
    ...
    foreach ($clogsSlaves as $l) {
        ...
        # folder?
        if ($l['isFolder'] == 1) {
            print "	<td><a href='" . create_link("subnets", $l['sectionId'], $l['id']) . "'>$l[description]</a></td>";
        } else {
            print "	<td><a href='" . create_link("subnets", $l['sectionId'], $l['id']) . "'>" . $Subnets->transform_to_dotted($l['subnet']) . "/$l[mask]</a></td>";
        }
        print "	<td>$l[description]</td>";
        print "	<td>" . _("$l[caction]") . "</td>";
        print "	<td>" . _("$l[cresult]") . "</td>";
