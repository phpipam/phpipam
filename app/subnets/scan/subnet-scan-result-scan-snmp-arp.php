...
if ($Subnets->check_permission($User->user, $_POST['subnetId']) != 3) {
    $Result->show("danger", _('You do not have permissions to modify hosts in this subnet') . "!", true);
}

# check for number of input values
$max = ini_get("max_input_vars");
if (sizeof($_POST) >= ini_get("max_input_vars")) {
    $Result->show("danger", _("Number of discovered hosts exceed maximum possible defined by php.ini - set to ") . " $max <hr>" . _("Please adjust your php.ini settings for value `max_input_vars`"), true);
}
...
foreach ($_POST as $key => $line) {
    ...
    elseif (substr($key, 0, 3) == "mac") {
        $res[substr($key, 3)]['mac'] = $line;
    } // device
    elseif (substr($key, 0, 6) == "device") {
        $res[substr($key, 6)]['switch'] = $line;
    } // description
    elseif (substr($key, 0, 11) == "description") {
        $res[substr($key, 11)]['description'] = $line;
    } // description
    elseif (substr($key, 0, 4) == "port") {
        $res[substr($key, 4)]['port'] = $line;
    } // dns name
    elseif (substr($key, 0, 8) == "hostname") {
        $res[substr($key, 8)]['hostname'] = $line;
    } // custom fields
