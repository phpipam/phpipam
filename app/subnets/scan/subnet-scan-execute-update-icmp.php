...
# set blank values
if (!isset($script_result->values->alive) || is_null($script_result->values->alive)) {
    $script_result->values->alive = [];
}
if (!isset($script_result->values->dead) || is_null($script_result->values->dead)) {
    $script_result->values->dead = [];
}
...
elseif (!isset($script_result->values)) {
    $Result->show("info", _("Subnet is empty") . "!", false);
} # no ip addresses - error in script
elseif (!isset($res)) {
    $Result->show("info", _("Error") . "!", false);
} # ok
else {
    # order by IP address
    ksort($res);
