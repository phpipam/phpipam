...
foreach ($slave_subnets as $slave_subnet) {
    ...
    if ($slave_subnet['isFull'] != 1) {
        ...
        if ($calculate['freehosts'] == 0) {
            $fullinfo = "<span class='badge badge1 badge2 badge4'>" . _("Full") . "</span>";
        }
    }
    ...
    if (isset($visible_fields)) {
        foreach ($visible_fields as $key => $field) {
            ...
            if ($field['type'] == "tinyint(1)") {
                if ($slave_subnet[$key] == "0") {
                    $html_custom = _("No");
                } elseif ($slave_subnet[$key] == "1") {
                    $html_custom = _("Yes");
                }
            } else {
    ...
    # allow requests
    if ($slave_subnet['allowRequests'] == 1) {
        print '<td class="allowRequests small hidden-xs hidden-sm hidden-md"><i class="fa fa-gray fa-check"></td>';
    } else {
        print '<td class="allowRequests small hidden-xs hidden-sm hidden-md">/</td>';
    }
