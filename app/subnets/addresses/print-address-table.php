...
foreach ($addresses as $a) {
    ...
    if (strlen($a->location) > 0) {
        $cnt_obj["location"]++;
    }        //not sure about this, because location is INT in Database
}

// check and remove empty
foreach ($cnt_obj as $field => $c) {
    if ($c == 0 && in_array($field, $selected_ip_fields)) {
        unset($selected_ip_fields[array_search($field, $selected_ip_fields)]);
    }
}
...
<h4 style="margin-top:40px;">
    ...
    if ($location === "customers") {
        ...
        print _("IP addresses in $location ");
    } elseif (@$orphaned) {
...
<!-- table -->
<table class="ipaddresses sortable sorted normalTable table table-condensed table-full table-top"
       data-cookie-id-table="ipaddresses">
    ...
    <tr class="th">
        ...
        if (in_array('switch', $selected_ip_fields) && $User->get_module_permissions("devices") > 0) {
            print "<th class='hidden-xs hidden-sm hidden-md'>" . _('Device') . "</th>";
        }
        if (in_array('port', $selected_ip_fields)) {
            print "<th class='hidden-xs hidden-sm hidden-md'>" . _('Port') . "</th>";
        }
        if (in_array('location', $selected_ip_fields) && $User->get_module_permissions("locations") > 0) {
            print "<th class='hidden-xs hidden-sm hidden-md'>" . _('Location') . "</th>";
        }
        if (in_array('owner', $selected_ip_fields)) {
            print "<th class='hidden-xs hidden-sm'>" . _('Owner') . "</th>";
        }
        if ($User->settings->enableCustomers == "1" && $cnt_obj["customer_id"] > 0 && $User->get_module_permissions("customers") > 0) {
            print "<th class='hidden-xs hidden-sm'>" . _('Customer') . "</th>";
        }
        // custom fields
        if (sizeof($custom_fields) > 0) {
    ...
    if ($addresses === false || sizeof($addresses) == 0) {
        if ($User->user->hideFreeRange != 1 && $subnet['isFull'] != "1") {
            $unused = $Addresses->find_unused_addresses($Subnets->transform_to_decimal($subnet_detailed['network']), $Subnets->transform_to_decimal($subnet_detailed['broadcast']), $subnet['mask'], $empty = true, false, false, $section['strictMode']);
        ...
        foreach ($addresses as $dummy) {
            ...
            if ($n == 0) {
                $unused = $Addresses->find_unused_addresses($Subnets->transform_to_decimal($subnet_detailed['network']), $addresses[$n]->ip_addr, $subnet['mask'], false, true);
            } # check unused space between IP addresses
            else {
                // compressed and dhcp?
                if ($addresses[$n - 1]->class == "compressed-range") {
                    $unused = $Addresses->find_unused_addresses($addresses[$n - 1]->stopIP, $addresses[$n]->ip_addr, $subnet['mask'], false, false, false);
                } // ignore /31 networks and /127
                elseif ($subnet['mask'] != 31 && $subnet['mask'] != 127) {
                    $unused = $Addresses->find_unused_addresses($addresses[$n - 1]->ip_addr, $addresses[$n]->ip_addr, $subnet['mask'], false, false, false);
                }
            }
            ...
            if (@$addresses[$n]->class == "compressed-range") {
                ...
                if ($subnet['pingSubnet'] == "1") {
                    ...
                    if ($addresses[$n]->excludePing == "1") {
                        $hStatus = "padded";
                        $hTooltip = "";
                    } elseif (is_null($addresses[$n]->lastSeen)) {
                        $hStatus = "neutral";
                        ...
                        $hStatus = "success";
                        $hTooltip = "rel='tooltip' data-container='body' data-html='true' data-placement='left' title='" . _("Address is alive") . "<hr>" . _("Last seen") . ": " . $addresses[$n]->lastSeen . "'";
                    } elseif ($tDiff < $statuses[1]) {
                        $hStatus = "warning";
                        $hTooltip = "rel='tooltip' data-container='body' data-html='true' data-placement='left' title='" . _("Address warning") . "<hr>" . _("Last seen") . ": " . $addresses[$n]->lastSeen . "'";
                    } elseif ($tDiff > $statuses[1]) {
                        $hStatus = "error";
                        $hTooltip = "rel='tooltip' data-container='body' data-html='true' data-placement='left' title='" . _("Address is offline") . "<hr>" . _("Last seen") . ": " . $addresses[$n]->lastSeen . "'";
                    } elseif ($addresses[$n]->lastSeen == "0000-00-00 00:00:00") {
                        $hStatus = "neutral";
                ...
                if (in_array('mac', $selected_ip_fields)) {
                    ...
                    // multicast ?
                    if ($User->settings->enableMulticast == "1" && $Subnets->is_multicast($addresses[$n]->ip_addr)) {
                        print "<td class='$mclass' style='white-space:nowrap;'>" . $addresses[$n]->mac . " $minfo $mobjects</td>";
                    } elseif (!empty($addresses[$n]->mac)) {
                        print "<td class='narrow'><i class='info fa fa-gray fa-sitemap' rel='tooltip' data-container='body' data-html='true' title='" . _('MAC') . ": " . $addresses[$n]->mac . $mac_vendor . "'></i></td>";
                    } else {
            ...
            if (strlen($User->settings->link_field) > 0) {
                ...
                if ($similar !== false) {
                    ...
                    foreach ($similar as $k => $s) {
                        ...
                        if (in_array('mac', $selected_ip_fields)) {
                            if (!empty($s->mac)) {
                                print "<td class='narrow'><i class='info fa fa-gray fa-sitemap' rel='tooltip' data-container='body' title='" . _('MAC') . ": " . $s->mac . "'></i></td>";
                            } else {
                                print "<td class='narrow'></td>";
                            }
                        }
                        ...
                        # print port
                        if (in_array('port', $selected_ip_fields)) {
                            print "<td class='hidden-xs hidden-sm hidden-md'>" . $s->port . "</td>";
                        }
                        # print location
                        if (in_array('location', $selected_ip_fields)) {
                            print "<td class='hidden-xs hidden-sm hidden-md'>" . $s->location . "</td>";
                        }
                        # print owner
                        if (in_array('owner', $selected_ip_fields)) {
                            print "<td class='hidden-xs hidden-sm'>" . $s->owner . "</td>";
                        }
                        # print custom fields
                        if (sizeof($custom_fields) > 0) {
                            foreach ($custom_fields as $myField) {
            ...
            ****************************************************/
            if ($n == $m) {
                if ($User->user->hideFreeRange != 1 && $subnet['isFull'] != "1") {
                    # compressed?
                    if (isset($addresses[$n]->stopIP)) {
                        $unused = $Addresses->find_unused_addresses($addresses[$n]->stopIP, $Subnets->transform_to_decimal($subnet_detailed['broadcast']), $subnet['mask'], false, false, true);
                    } else {
                        $unused = $Addresses->find_unused_addresses($addresses[$n]->ip_addr, $Subnets->transform_to_decimal($subnet_detailed['broadcast']), $subnet['mask'], false, false, true);
                    }
