...
if ($action == "add") {
    $btnName = _("add");
    $act = "add";
} elseif ($action == "all-add") {
    $btnName = _("add");
    $act = "add";
} elseif ($action == "edit") {
    $btnName = _("edit");
    $act = "edit";
} elseif ($action == "all-edit") {
    $btnName = _("edit");
    $act = "edit";
...
//validatemac
        function validate_mac(ip, mac, sectionId, vlanId, id) {
            $.post('app/tools/multicast-networks/validate_mac.php', {
                ip: ip,
                mac: mac,
                sectionId: sectionId,
                vlanId: vlanId,
                ...
                if (data === "True") {
                    ...
                    if ($('#helpBlock2').length) {
                        $("#helpBlock2").html(data);
                    } else {
                        $("input[name=mac]").parent().append("<span id='helpBlock2' class='help-block'>" + data + "</span>");
                    }
                }
            });
        }
    ...
    <form class="editipaddress" role="form" name="editipaddress">
        ...
        <table id="editipaddress" class="table table-noborder table-condensed">
            ...
            <tr>
                ...
                <td>
                    <div class="input-group">
                        <input type="text" name="ip_addr" class="ip_addr form-control input-sm"
                               value="<?php print $Subnets->transform_address($address['ip_addr'], "dotted");;
                               if (is_numeric($_POST['stopIP']) > 0) print "-" . $Subnets->transform_address($_POST['stopIP'], "dotted"); ?>"
                               placeholder="<?php print _('IP address'); ?>">
                        <span class="input-group-addon" style="border-left:none;">
    			<a class="ping_ipaddress ping_ipaddress_new" data-subnetid="<?php print $subnetId; ?>" data-id=""
                   href="#" rel="tooltip" data-container="body" title="" data-original-title="Check availability">
 					<i class="fa fa-gray fa-cogs"></i>
    			</a>
 			</span>
                        <span class="input-group-addon">
    			<i class="fa fa-gray fa-info" rel="tooltip" data-html='true' data-placement="right"
                   title="<?php print _('You can add,edit or delete multiple IP addresses<br>by specifying IP range (e.g. 10.10.0.0-10.10.0.25)'); ?>"></i>
    		</span>
                    </div>
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="description" class="ip_addr form-control input-sm"
                           value="<?php if (isset($address['description'])) {
            ...
            if (in_array('mac', $selected_ip_fields)) {
                ...
                if ($User->settings->enableMulticast == 1) {
                    ...
                    if (isset($mcast_err)) {
                        $mcast_class = 'has-error';
                        $mcast_help_block = "<span id='helpBlock2' class='help-block'>$mcast_err</span>";
                    } else {
                        $mcast_class = "";
                        $mcast_help_block = "";
                    }
            ...
            <tr>
                ...
                <td>
                    <input type="checkbox" name="is_gateway" class="input-switch"
                           value="1" <?php if (@$address['is_gateway'] == 1) print "checked"; ?> <?php print $delete; ?>>
                </td>
            </tr>
...
<div class='col-xs-12 col-md-6'>
    ...
    <table id='editipaddress' class='table table-noborder table-condensed'>
        ...
        foreach ($custom_fields as $field) {
            ...
            # set default value !
            if ($_POST['action'] == "add" || $_POST['action'] == "all-add") {
            ...
            elseif ($field['type'] == "date" || $field['type'] == "datetime") {
                ...
                if (!isset($address[$field['name']])) {
                    print ' <input type="text" class="' . $class . ' form-control input-sm input-w-auto" data-format="' . $format . '" name="' . $field['nameNew'] . '" maxlength="' . $size . '" ' . $delete . ' rel="tooltip" data-placement="right" title="' . $field['Comment'] . '">' . "\n";
                } else {
                    print ' <input type="text" class="' . $class . ' form-control input-sm input-w-auto" data-format="' . $format . '" name="' . $field['nameNew'] . '" maxlength="' . $size . '" value="' . $address[$field['name']] . '" ' . $delete . ' rel="tooltip" data-placement="right" title="' . $field['Comment'] . '">' . "\n";
                }
            } //boolean
                ...
                <td>
                    <div class='checkbox info2'>
                        <input type="checkbox" name="unique"
                               value="1" <?php print $delete; ?>><?php print _('Unique hostname'); ?>
                    </div>
                </td>
            </tr>
        <?php } ?>
        ...
        if ($subnet['mask'] < 31 && ($action == 'add' || substr($action, 0, 4) == "all-") && $type == "IPv4") { ?>
            ...
            <tr>
                ...
                <td>
                    <div class='checkbox info2'>
                        <input type="checkbox" name="nostrict"
                               value="yes"><?php print _('Permit adding network/broadcast as IP'); ?>
                    </div>
                </td>
            </tr>
        <?php } ?>
...
<div class="pFooter">
    <div class="btn-group">
        ...
        ?>
        <button class="btn btn-sm btn-default <?php if ($action == "delete") {
            print "btn-danger";
        } else {
            print "btn-success";
        } ?>" id="editIPAddressSubmit" data-action='<?php print $action; ?>'><i class="fa <?php if ($act == "add") {
                print "fa-plus";
            } elseif ($act == "delete") {
                print "fa-trash-o";
            } else {
                print "fa-check";
            } ?>"></i> <?php print ucwords($btnName); ?> IP
        </button>
    </div>
