	...
	print "<table class='ipaddress_subnet table-condensed table-auto'>";
    	...
    	if( $subnet_permission > 1) {
    		...
    		else
    		{
    			...
    			//share
    			if($User->settings->tempShare==1) {
                    print "     <a class='btn btn-xs btn-default open_popup' data-script='app/tools/temp-shares/edit.php' data-class='700' data-action='edit' data-id='$address[id]' data-type='ipaddresses' data-container='body' rel='tooltip' title='"._('Temporary share address')."'><i class='fa fa-share-alt'></i></a>";
                }
    		}
    	}
    	# write not permitted
