...
if (is_array($required_ip_fields) && $action != "delete") {
    ...
    if ($User->settings->enableLocations == "0") {
        unset($required_ip_fields['location_item']);
    }
...
if (strlen($address['hostname']) > 0) {
    $address['hostname'] = str_replace(" ", "", $address['hostname']);
}
...
if (sizeof($custom_fields) > 0 && $action != "delete") {
    foreach ($custom_fields as $field) {
        ...
        if (isset($address[$field['nameTest']])) {
            $address[$field['name']] = $address[$field['nameTest']];
        }
...
if (strlen(strstr($address['ip_addr'], "-")) > 0) {
    ...
    if (isset($address['unique'])) {
        if ($address['unique'] == 1 && strlen($address['hostname']) > 0) {
            # check if unique
            if (!$Addresses->is_hostname_unique($address['hostname'])) {
    ...
    if ($verify) {
        $Result->show("danger", _('Error') . ": $verify ($address[ip_addr])", true);
    } else {
        ...
        if ($action == "add") {
            if ($Addresses->address_exists($address['ip_addr'], $address['subnetId'])) {
                $Result->show("danger", _('IP address') . " $address[ip_addr] " . _('already existing in selected network') . '!', true);
            }
        }
        ...
        if ($action == "move") {
            ...
            if ($Addresses->address_exists($address['ip_addr'], $address['newSubnet'])) {
                $Result->show("danger", _('IP address') . " $address[ip_addr] " . _('already existing in selected network') . '!', true);
            }
        }
        # multicast check
