...
for ($m = $start_visual; $m <= $stop_visual; $m = gmp_strval(gmp_add($m, 1))) {
    ...
    if (array_key_exists((string)$m, $visual_addresses)) {
        ...
        if (strlen($visual_addresses[$m]['hostname']) > 0) {
            $title .= "<br>" . $visual_addresses[$m]['hostname'];
        }
        if (strlen($visual_addresses[$m]['description']) > 0) {
            $title .= "<br>" . $visual_addresses[$m]['description'];
        }
    ...
    if ($subnet_permission > 1) {
        print "<span class='ip-$class modIPaddr' 	style='background:$background;color:$foreground' data-action='$action' rel='tooltip' title='$title' data-position='top' data-html='true' data-subnetId='" . $subnet['id'] . "' data-id='$id'>." . substr(strrchr($Subnets->transform_to_dotted($m), "."), 1) . "</span>";
    } else {
        print "<span class='ip-$class '  			style='background:$background;color:$foreground' data-action='$action' data-subnetId='" . $subnet['id'] . "' data-id='$id'>." . substr(strrchr($Subnets->transform_to_dotted($m), "."), 1) . "</span>";
    }
}
print "</div>";
