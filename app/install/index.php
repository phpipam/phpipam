...
<head>
    ...
    <?php if ($User->settings->theme != "white") { ?>
        <link rel="stylesheet" type="text/css"
              href="css/bootstrap/bootstrap-custom-<?php print $User->settings->theme; ?>.css?v=<?php print SCRIPT_PREFIX; ?>">
    <?php } ?>
...
<div class="wrapper">
    ...
    <div class="jqueryError">
        ...
        <br>
        <a href="<?php print create_link(null, null, null, null, null, true); ?>" class="btn btn-sm btn-default"
           id="hideError" style="margin-top:0px;">Hide</a>
    </div>
        ...
        <div class="container-fluid" id="mainContainer">
            <div class='container' id='dashboard'>
                ...
                if (!isset($_GET['section'])) {
                    ...
                    if (@$_GET['section'] == "select_type") {
                        include(dirname(__FILE__) . "/select_install_type.php");
                    } // section error
                    elseif (@$_GET['section'] == "sql_error") {
                        include(dirname(__FILE__) . "/sql_error.php");
                    } // check if subnetId == configure than already installed
                    elseif (@$_GET['subnetId'] == "configure") {
                        include(dirname(__FILE__) . "/postinstall_configure.php");
                    } // set installation type
                    else {
                        // validate install type
                        $install_types = ["install_automatic", "install_manual", "install_mysqlimport"];
                        if (!in_array($_GET['section'], $install_types)) {
                            $Result->show("danger", "Invalid request", false);
                        } else {
                            // verify that page exists
                            if (!file_exists(dirname(__FILE__) . "/$_GET[section].php")) {
                                include("invalid_install_type.php");
                            } else {
