...
elseif ($User->settings->enableRouting != "1") {
    ...
    if (isset($_GET['sPage'])) {
        ...
        # include
        if ($_GET['subnetId'] == "bgp") {
            include(dirname(__FILE__) . "/bgp/details.php");
        } elseif ($_GET['subnetId'] == "ospf") {
            include(dirname(__FILE__) . "/ospf/details.php");
        } else {
            $Result->show("danger", _("Invalid routing module."), false);
        }
    } # all entries
    else {
        # default
        ...
        # include
        if ($_GET['subnetId'] == "bgp") {
            include(dirname(__FILE__) . "/bgp/all.php");
        } elseif ($_GET['subnetId'] == "ospf") {
            include(dirname(__FILE__) . "/ospf/all.php");
        } else {
            $Result->show("danger", _("Invalid routing module."), false);
        }
    }
