                ...
                foreach ($object_groups as $t => $ob) {
                    ...
                    if (sizeof($ob) > 0) {
                        foreach ($ob as $o) {
                            ...
                            // link
                            if ($o->type == "devices") {
                                $href = create_link("tools", "devices", $o->id);
                            } elseif ($o->type == "subnets") {
                                $href = create_link("subnets", $o->sectionId, $o->id);
                            } elseif ($o->type == "addresses") {
                                $href = create_link("subnets", $subnet->sectionId, $subnet->id, "address-details", $o->id);
                            } elseif ($o->type == "circuit") {
                                $href = create_link("tools", "circuits", $o->id);
                            } else {
                                $href = create_link("tools", "racks", $o->id);
                            }
