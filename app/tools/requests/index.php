...
<?php
	...
	foreach($requests as $k=>$request) {
		...
		if($subnet===false) {
			...
			print '	<td>'. $Tools->strip_xss($request['ip_addr']) .'</td>'. "\n";
			print '	<td>'. $Subnets->transform_to_dotted($subnet['subnet']) .'/'. $subnet['mask'] .' ('. $subnet['description'] .')</td>'. "\n";
			print '	<td>'. $Tools->strip_xss($request['hostname']) .'</td>'. "\n";
			print '	<td>'. $Tools->strip_xss($request['description']) .'</td>'. "\n";
			print '	<td>'. $Tools->strip_xss($request['requester']) .'</td>'. "\n";
			print '	<td>'. $Tools->strip_xss($request['comment']) .'</td>'. "\n";
			print '</tr>'. "\n";
		}
	}
?>
