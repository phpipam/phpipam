...
if ($_GET["sPage"]!=="all") {
 	$group=$Tools->fetch_object("vlanGroups", "id", $_GET["sPage"]);
	 	# fetch l2 domain
	$vlan_domain = $Tools->fetch_object("vlanDomains", "id", $group->domainId);
	if($vlan_domain===false)			{ $Result->show("danger", _("Invalid ID"), true); }
}
else{
	$group = array();
	$group["name"] = "All VLANs";
	$group["firstVlan"] = 1;
	$group["lastVlan"] = $User->settings->vlanMax;
	$group["domainId"] = $_GET["ipaddrid"];
	$group = (object) $group;
	# fetch l2 domain
	$vlan_domain = $Tools->fetch_object("vlanDomains", "id", $_GET["ipaddrid"]);
	if($vlan_domain===false)			{ $Result->show("danger", _("Invalid ID"), true); }
}
...
# Filter out vlans outside the group range
$vlansTemp = array();
foreach ($vlans as $vlan){
    if ($vlan[0]->number >= $group->firstVlan && $vlan[0]->number <= $group->lastVlan) {
        $vlansTemp[] = $vlan;
    }
}
$vlans = $vlansTemp;
...
# title
print "<h4>"._('Available VLANs in domain: ')." $vlan_domain->name | Group:  $group->name </h4>";
print "<hr>";
...
<div class="btn-group" style="margin-bottom:10px;">
    ...
    print "<a class='btn btn-sm btn-default' href='".create_link($_GET['page'], $_GET['section'], $group->domainId)."'><i class='fa fa-angle-left'></i> "._('Domain VLAN Groups')."</a>";
    }
    ?>
    <?php
    // l2 domains
...
# no VLANS?
if($vlans==false) {
    $Result->show("info", _("No VLANS configured in this group"), false);
}
else {
    # table
    ...
    print "<tbody>";
    $m = 0;
    $closingFreeRangePrinted = false;
    $lastPrinted = 0;
    foreach ($vlans as $vlan) {
        //extract vlan number
        $number;
        foreach($vlan as $k=>$v) {
            if($k==0) $number = $vlan[0]->number;
        }

        // show free vlans - start
        if($User->user->hideFreeRange!=1) {
            if($m==0 && $vlan[0]->number!=$group->firstVlan)	{
                print "<tr class='success'>";
                print "<td></td>";
                print "<td colspan='".(5+$csize)."'><btn class='btn btn-xs btn-default editVLAN' data-action='add' data-domain='".$vlan_domain->id."'  data-number='1'><i class='fa fa-plus'></i></btn> "._('VLAN')." $group->firstVlan - ".($number-1)." (".($number - $group->firstVlan)." "._('free').")</td>";
                print "</tr>";
            }
            ...
            if($m>0)	{
                if( (($vlans[$m][0]->number)-($vlans[$m-1][0]->number)-1) > 0 ) {
                    ...
                    if( (($vlans[$m][0]->number)-($vlans[$m-1][0]->number)-1) ==1 ) {
                        print "<td colspan='".(5+$csize)."'><btn class='btn btn-xs btn-default editVLAN' data-action='add' data-domain='".$vlan_domain->id."' data-number='".($vlans[$m][0]->number -1)."'><i class='fa fa-plus'></i></btn> "._('VLAN')." ".($vlans[$m][0]->number -1)."</td>";
                    }
                    else {
                        print "<td colspan='".(5+$csize)."'><btn class='btn btn-xs btn-default editVLAN' data-action='add' data-domain='".$vlan_domain->id."' data-number='".($vlans[$m-1][0]->number+1)."'><i class='fa fa-plus'></i></btn> "._('VLAN')." ".($vlans[$m-1][0]->number+1)." - ".($vlans[$m][0]->number-1)." (".(($vlans[$m][0]->number)-($vlans[$m-1][0]->number)-1)." "._('free').")</td>";
                    }
                    print "</tr>";
                }
            }
        ...
        if(sizeof($vlan)>0) {
            foreach($vlan as $k=>$v) {
                ...
                if($k==0) {
                    ...
                    print "	<td><a class='btn btn-xs btn-default' href='".create_link($_GET['page'], $_GET['section'], $vlan_domain->id, $vlan[0]->vlanId)."'><i class='fa fa-cloud prefix'></i> ".$vlan[0]->number."</td>";
                    print "	<td>".$vlan[0]->name."</td>";

                    if($vlan[0]->description != "") print "	<td>".$vlan[0]->description."</td>";
                    if($vlan[0]->description == "") print "	<td> / </td>";
        ...
        if($User->user->hideFreeRange!=1) {
            if($m==(sizeof($vlans)-1)) {
                if($group->lastVlan > $vlan[0]->number) {
                    print "<tr class='success'>";
                    print "<td></td>";
                    print "<td colspan='".(5+$csize)."'><btn class='btn btn-xs btn-default editVLAN' data-action='add' data-domain='".$vlan_domain->id."'  data-number='".($vlan[0]->number+1)."'><i class='fa fa-plus'></i></btn> "._('VLAN')." ".($vlan[0]->number+1)." - ".$group->lastVlan." (".(($group->lastVlan)-($vlan[0]->number))." "._('free').")</td>";
                    print "</tr>";
                }
            }
        }
        ...
        if ($number > $group->lastVlan && $lastPrinted != $group->lastVlan && $closingFreeRangePrinted == false) {
            # show free vlans - last
            if($User->user->hideFreeRange!=1) {
                print "<tr class='success'>";
                print "<td></td>";
                print "<td colspan='".(5+$csize)."'><btn class='btn btn-xs btn-default editVLAN' data-action='add' data-domain='".$vlan_domain->id."'  data-number='".($lastPrinted+1)."'><i class='fa fa-plus'></i></btn> "._('VLAN')." ".($lastPrinted+1)." - ".$group->lastVlan." (".(($group->lastVlan)-($lastPrinted))." "._('free').")</td>";
                print "</tr>";
            }
            $closingFreeRangePrinted = true;
        }
        if ($number >= $group->firstVlan && $number <= $group->lastVlan) $lastPrinted = $number;
    }
    print "</tbody>";
    print '</table>';
}
