...
if ($_POST['action'] == "add" || $_POST['action'] == "edit") {
    ...
    if (!is_numeric($_POST['number'])) {
        $Result->show("danger", _("Number must be numeric"), true);
    }
    
    // check prefix
    $prefix = $Tools->fetch_object("pstnPrefixes", "id", $_POST['prefix']);
    if ($prefix === false) {
        $Result->show("danger", _("Invalid prefix"), true);
    }
    
    // duplicate check
    if ($_POST['action'] == "add")
        if ($Tools->check_number_duplicates($prefix->id, $_POST['number'])) {
            $Result->show("danger", _("Duplicate number"), true);
        }
...
if (!$Admin->object_modify("pstnNumbers", $_POST['action'], "id", $values)) {
    $Result->show("danger", _("Number $_POST[action] failed"), false);
} else {
    $Result->show("success", _("Number $_POST[action] successful"), false);
}