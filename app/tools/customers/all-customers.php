...
// result
	...
	foreach ($customers as $customer) {
		// print details
		print '<tr>'. "\n";
		print "	<td><strong><a class='btn btn-sm btn-default' href='".create_link($GET->page,"customers",unescape_input($customer->title))."'>$customer->title</a></strong></td>";
        ...
        if($User->get_module_permissions ("customers")>=User::ACCESS_R) {
            ...
            $links[] = ["type"=>"link", "text"=>_("Show customer"), "href"=>create_link($GET->page, "customers", unescape_input($customer->title)), "icon"=>"eye", "visible"=>"dropdown"];
            $links[] = ["type"=>"divider"];
        }
