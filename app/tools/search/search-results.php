...
// all are off?
if (is_blank($GET->addresses) && is_blank($GET->subnets) && is_blank($GET->vlans) && is_blank($GET->vrf) && is_blank($GET->pstn) && is_blank($GET->circuits) && is_blank($GET->customers) && is_blank($GET->locations)) {
...
// ok, search results print
	...
	if (!is_blank($GET->ip)) {
		...
		// locations
		if ($GET->locations == "on" && $User->get_module_permissions("locations") >= User::ACCESS_R) {
			require(dirname(__FILE__) . '/search_results/search-results_locations.php');
		}
	}
