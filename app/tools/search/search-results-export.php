...
if (@$_REQUEST['addresses'] == "on") {
    $result_addresses = $Tools->search_addresses($search_term, $search_term_edited['high'], $search_term_edited['low'], $custom_address_fields);
} else {
    $result_addresses = [];
}
# search subnets
if (@$_REQUEST['subnets'] == "on") {
    $result_subnets = $Tools->search_subnets($search_term, $search_term_edited['high'], $search_term_edited['low'], $_REQUEST['ip'], $custom_subnet_fields);
} else {
    $result_subnets = [];
}
# search vlans
if (@$_REQUEST['vlans'] == "on" && $User->get_module_permissions("vlan") > 0) {
    $result_vlans = $Tools->search_vlans($search_term, $custom_vlan_fields);
} else {
    $result_vlans = [];
}
# search vrf
if (@$_REQUEST['vrf'] == "on" && $User->get_module_permissions("vrf") > 0) {
    $result_vrf = $Tools->search_vrfs($search_term, $custom_vrf_fields);
} else {
    $result_vrf = [];
}
# search circuits
if (@$_REQUEST['circuits'] == "on" && $User->get_module_permissions("circuits") > 0) {
    $result_circuits = $Tools->search_circuits($search_term, $custom_circuit_fields);
} else {
    $result_circuits = [];
}
if (@$_REQUEST['circuits'] == "on" && $User->get_module_permissions("circuits") > 0) {
    $result_circuits_p = $Tools->search_circuit_providers($search_term, $custom_circuit_p_fields);
} else {
    $result_circuits_p = [];
}

# search customers
if (@$_REQUEST['customers'] == "on" && $User->get_module_permissions("customers") > 0) {
    $result_customers = $Tools->search_customers($search_term, $custom_vrf_fields);
} else {
    $result_customers = [];
}
...
if (is_array($result_addresses) && sizeof($result_addresses) > 0) {
    ...
    if (in_array('state', $selected_ip_fields)) {
        $worksheet->write($lineCount, $x, _('state'), $format_title);
        $x++;
    } else {
        $colSpan--;
    }
    # description, note
    $worksheet->write($lineCount, $x, _('description'), $format_title);
    $x++;
    $worksheet->write($lineCount, $x, _('hostname'), $format_title);
    $x++;
    # switch
    if (in_array('switch', $selected_ip_fields) && $User->get_module_permissions("devices") > 0) {
        $worksheet->write($lineCount, $x, _('device'), $format_title);
        $x++;
    } else {
        $colSpan--;
    }
    ...
    if (in_array('port', $selected_ip_fields)) {
        $worksheet->write($lineCount, $x, _('port'), $format_title);
        $x++;
    } else {
        $colSpan--;
    }
    ...
    if (in_array('location', $selected_ip_fields) && $User->get_module_permissions("locations") > 0) {
        $worksheet->write($lineCount, $x, _('location'), $format_title);
        $x++;
    } else {
        $colSpan--;
    }
    ...
    if (in_array('owner', $selected_ip_fields)) {
        $worksheet->write($lineCount, $x, _('owner'), $format_title);
        $x++;
    } else {
        $colSpan--;
    }
    ...
    if (in_array('mac', $selected_ip_fields)) {
        $worksheet->write($lineCount, $x, _('mac'), $format_title);
        $x++;
    } else {
        $colSpan--;
    }
    ...
    if (in_array('note', $selected_ip_fields)) {
        $worksheet->write($lineCount, $x, _('note'), $format_title);
        $x++;
    } else {
        $colSpan--;
    }
    ...
    foreach ($result_addresses as $ip) {
        ...
        if ($subnet_permission > 0) {
            ...
            $x++;
            $worksheet->write($lineCount, $x, $ip['hostname']);
            $x++;
            # switch
            if (in_array('switch', $selected_ip_fields) && $User->get_module_permissions("devices") > 0) {
            ...
            # port
            if (in_array('port', $selected_ip_fields)) {
                $worksheet->write($lineCount, $x, $ip['port']);
                $x++;
            }
            # location
            if (in_array('location', $selected_ip_fields) && $User->get_module_permissions("locations") > 0) {
                $worksheet->write($lineCount, $x, $ip['location']);
                $x++;
            }
            # owner
            if (in_array('owner', $selected_ip_fields)) {
                $worksheet->write($lineCount, $x, $ip['owner']);
                $x++;
            }
            # mac
            if (in_array('mac', $selected_ip_fields)) {
                $worksheet->write($lineCount, $x, $ip['mac']);
                $x++;
            }
            # note
            if (in_array('note', $selected_ip_fields)) {
                $worksheet->write($lineCount, $x, $ip['note']);
                $x++;
            }
            ...
            if (sizeof($custom_address_fields) > 0) {
                foreach ($custom_address_fields as $myField) {
                    $worksheet->write($lineCount, $x, $ip[$myField['name']]);
                    $x++;
                }
            }
...
if (is_array($result_subnets) && sizeof($result_subnets) > 0) {
    ...
    foreach ($result_subnets as $line) {
        ...
        if ($line['masterSubnetId'] == 0) {
            ...
            # folder?
            if ($line['masterSubnetId']['isFolder'] == 1) {
                $line['masterSubnetId'] = $line['masterSubnetId']['description'];
            } else {
                $line['masterSubnetId'] = $Subnets->transform_to_dotted($line['masterSubnetId']['subnet']) . '/' . $line['masterSubnetId']['mask'] . "(" . $line['masterSubnetId']['description'] . ")";
            }
        }
        //allowRequests
        if ($line['allowRequests'] == 1) {
            $line['allowRequests'] = 'yes';
        } else {
            $line['allowRequests'] = '';
        }
