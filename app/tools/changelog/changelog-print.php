...
if (isset($_GET['subnetId'])) {
    ...
    if (!!preg_match('/[^A-Za-z0-9.#*% <>_ \\-$]/', $_GET['subnetId'])) {
        $Result->show("danger", _("Invalid search string") . "!", true);
    }
}
...
# numeric check
if (!is_numeric($input_climit) || $input_climit < 1) {
    $Result->show("danger", _("Invalid limit") . "!", true);
}

# get clog entries
if (empty($input_cfilter)) {
    $clogs = $Log->fetch_all_changelogs(false, "", $input_climit);
} else {
    $clogs = $Log->fetch_all_changelogs(true, $input_cfilter, $input_climit);
}
...
if (sizeof($clogs) == 0) {
    ...
    if (sizeof($clogs) == $input_climit) {
        $Result->show("warning alert-absolute", _("Output has been limited to last $input_climit lines") . "!", false);
    }
