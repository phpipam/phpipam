...
if ($User->settings->enableIPrequests==1) {
	...
	# parse
	if ($User->is_admin()==false && isset($requests)) {
...
<nav class="navbar navbar-default" id="menu-navbar" role="navigation">
	...
	<ul class="nav navbar-nav navbar-right hidden-xs hidden-sm icon-ul">
		...
		<!-- Dash lock/unlock -->
		<?php if($_GET['page']=="dashboard" && !($User->is_admin()!==true && (strlen($User->user->groups)==0 || $User->user->groups==="null") ) ) { ?>
			<li class="w-lock">
    	...
    	<!-- DB verification -->
		<?php
		if($User->is_admin() && $User->settings->dbverified==0) {
			//check
		...
		if( $User->is_admin() && (strtotime(date("Y-m-d H:i:s")) - strtotime($User->settings->vcheckDate)) > 604800 ) {
