...
if (strlen($_POST['option']) == 0) {
    $Result->show("danger", _('Value cannot be empty'), true);
}
# strip tags - XSS
...
if ($_POST['type'] == "type") {
    if ($_POST['option'] == "Default") {
        $Result->show("danger", _('Default value cannot be deleted'), true);
    }
}
...
# execute
if (!$Admin->object_modify("circuitTypes", $_POST['action'], "id", $values)) {
    $Result->show("danger", _("Option $_POST[action] error"), true);
} else {
    $Result->show("success", _("Option $_POST[action] success"), false);
}
