...
if (sizeof($id_list) != sizeof(array_unique($id_list))) {
    $Result->show("danger", _('Remove duplicates of circuit') . '!', true);
}
if (($circuit['action'] == "add" && sizeof($id_list) == 0)) {
    $Result->show("danger", _('No circuits selected') . '!', true);
}
...
if (!$Admin->object_modify("circuitsLogical", $circuit['action'], "id", $values)) {
    ...
    if ($circuit['action'] != 'add') {
        try {
            $Database->deleteObjectsByIdentifier("circuitsLogicalMapping", "logicalCircuit_id", $circuit['id']);
        } catch (Exception $e) {
