...
# IDs must be numeric
if ($circuit['action'] != "add" && !is_numeric($circuit['id'])) {
    $Result->show("danger", _("Invalid ID"), true);
}
if (!is_numeric($circuit['provider'])) {
    $Result->show("danger", _("Invalid ID"), true);
}
...
# validate provider
if ($Tools->fetch_object("circuitProviders", "id", $circuit['provider']) === false) {
    $Result->show("danger", _('Invalid provider') . '!', true);
}
...
foreach ($all_types as $t) {
    array_push($type_id_array, $t->id);
}

if (!in_array($circuit['type'], $type_id_array)) {
    $Result->show("danger", _('Invalid type') . '!', true);
}

# status
$statuses = ["Active", "Inactive", "Reserved"];
if (!in_array($circuit['status'], $statuses)) {
    $Result->show("danger", _('Invalid status') . '!', true);
}
...
if ($_POST['action'] == 'delete') {
    ...
    if (!empty($logical_circuit_array)) {
        $Result->show("danger", _('Circuit is currently used in a larger logical circuit') . '!', true);
    }
...
if ($circuit['device1'] == "0") {
    ...
    if ($Tools->fetch_object("devices", "id", $deviceId) === false) {
        $Result->show("danger", _('Invalid device A') . '!', true);
    }
    // save
    ...
    if ($Tools->fetch_object("locations", "id", $locationId) === false) {
        $Result->show("danger", _('Invalid location A') . '!', true);
    }
    // save
    $circuit['device1'] = 0;
...
if ($circuit['device2'] == "0") {
    ...
    if ($Tools->fetch_object("devices", "id", $deviceId) === false) {
        $Result->show("danger", _('Invalid device B') . '!', true);
    }
    // save
    ...
    if ($Tools->fetch_object("locations", "id", $locationId) === false) {
        $Result->show("danger", _('Invalid location B') . '!', true);
    }
    // save
    $circuit['device2'] = 0;
...
if (sizeof($custom) > 0) {
    foreach ($custom as $myField) {
        ...
        if (isset($circuit[$myField['nameTest']])) {
            $circuit[$myField['name']] = $circuit[$myField['nameTest']];
        }
        ...
        //not null!
        if ($myField['Null'] == "NO" && strlen($circuit[$myField['name']]) == 0) {
...
if (!$Admin->object_modify("circuits", $circuit['action'], "id", $values)) {
    ...
    $Result->show("success", _("Circuit $circuit[action] successfull") . '!', false);
}
