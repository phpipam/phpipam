...
# ID must be numeric
if ($provider['action'] != "add" && !is_numeric($provider['providerid'])) {
...
if (sizeof($custom) > 0) {
    foreach ($custom as $myField) {
        ...
        if (isset($provider[$myField['nameTest']])) {
            $provider[$myField['name']] = $provider[$myField['nameTest']];
        }
        ...
        //not null!
        if ($myField['Null'] == "NO" && strlen($provider[$myField['name']]) == 0) {
...
if (!$Admin->object_modify("circuitProviders", $provider['action'], "id", $values)) {
    ...
    $Result->show("success", _("Provider $provider[action] successfull") . '!', false);
}
