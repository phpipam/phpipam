...
<!-- Add new -->
<button class='btn btn-sm btn-default open_popup' data-script='app/admin/users/edit.php' data-class='700'
        data-action='add'><i class='fa fa-plus'></i> <?php print _('Create user'); ?></button>
...
<table id="userPrint1" class="table sorted table-striped table-top table-td-top" data-cookie-id-table="admin_users">
    ...
    foreach ($users as $user) {
        ...
        if ($auth_method === false) {
            print "<span class='text-muted'>No auth method</span>";
        } else {
            print $auth_method->type . " <span class='text-muted'>(" . $auth_method->description . "</a>)";
        }
        print "</span></td>";
            ...
            foreach ($custom as $field) {
                if (!in_array($field['name'], $ffields)) {
                    ...
                    if ($field['type'] == "tinyint(1)") {
                        if ($user[$field['name']] == "0") {
                            print _("No");
                        } elseif ($user[$field['name']] == "1") {
                            print _("Yes");
                        }
                    } //text
                    elseif ($field['type'] == "text") {
                        if (strlen($user[$field['name']]) > 0) {
