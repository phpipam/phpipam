...
<div class="alert alert-info alert-absolute"><?php print _('You can add additional custom fields to IP addresses and subnets (like CustomerId, location, ...)'); ?>
    .
</div>
    ...
    foreach ($custom_fields as $k => $cf) {
        ...
        if (sizeof($cf) == 0) {
            ...
            foreach ($cf as $f) {
                ...
                # ordering
                if ((($m + 1) != $size)) {
                    print "<td style='width:10px;'><button class='btn btn-xs btn-default down' data-direction='down' data-table='$table' rel='tooltip' title='Move down' data-fieldname='" . $custom_fields_numeric[$table][$m]['name'] . "' data-nextfieldname='" . $custom_fields_numeric[$table][$m + 1]['name'] . "'><i class='fa fa-chevron-down'></i></button></td>";
                } else {
                ...
                # NULL
                if (@$f['Null'] == "NO") {
                    print "<td>" . _('Required') . "</td>";
                } else {
                    print "<td></td>";
                }
                
                # visible
                if (in_array($f['name'], $ffields)) {
                    print "<td><span class='text-danger'>" . _('No') . "</span></td>";
                } else {
                    print "<td><span class='text-success'>" . _('Yes') . "</span></td>";
                }
                ...
                if ((is_numeric(substr($f['name'], 0, 1))) || (!preg_match('/^(\p{L}|\p{N})[(\p{L}|\p{N}) _.-]+$/u', $f['name']))) {
