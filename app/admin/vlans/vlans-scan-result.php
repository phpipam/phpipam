...
foreach ($_POST as $key => $line) {
    ...
    if (substr($key, 0, 4) == "name") {
        $res[substr($key, 4)]['name'] = $line;
    } // mac
    elseif (substr($key, 0, 6) == "number") {
        $res[substr($key, 6)]['number'] = $line;
    } // device
    elseif (substr($key, 0, 8) == "domainId") {
        $res[substr($key, 8)]['domainId'] = $line;
    } // description
...
if (sizeof($res) > 0) {
    ...
    foreach ($res as $r) {
        ...
        # insert vlans
        if (!$Admin->object_modify("vlans", "add", "vlanId", $values)) {
