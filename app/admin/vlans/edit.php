...
<div class="pContent">
    <form id="<?php print $formId; ?>">
        ...
        <table id="vlanManagementEdit2" class="table table-noborder table-condensed">
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="number form-control input-sm" name="number"
                           placeholder="<?php print _('VLAN number'); ?>"
                           value="<?php print $Tools->strip_xss(@$vlan['number']); ?><?php print @$_POST['vlanNum']; ?>" <?php print $readonly; ?>>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="name form-control input-sm" name="name"
                           placeholder="<?php print _('VLAN name'); ?>"
                           value="<?php print $Tools->strip_xss(@$vlan['name']); ?>" <?php print $readonly; ?>>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="description form-control input-sm" name="description"
                           ...
                           value="<?php print $Tools->strip_xss(@$vlan['description']); ?>" <?php print $readonly; ?>>
            ...
            if ($User->settings->enableCustomers == 1 && $User->get_module_permissions("customers") > 0) {
                ...
                if ($customers != false) {
                    foreach ($customers as $customer) {
                        if ($customer->id == $vlan['customer_id']) {
    ...
    if ($_POST['action'] == "delete") {
        $Result->show("warning", _('Warning') . ':</strong> ' . _('removing VLAN will also remove VLAN reference from belonging subnets') . "!", false);
    }
    ?>
</div>
...
<div class="pFooter">
    <div class="btn-group">
        <button class="btn btn-sm btn-default <?php if (isset($_POST['fromSubnet'])) {
            print "hidePopup2";
        } else {
            print "hidePopups";
        ...
        <button class="btn btn-sm btn-default <?php if ($_POST['action'] == "delete") {
            ...
            print "btn-success";
        } ?> vlanManagementEditFromSubnetButton" id="editVLANsubmit"><i class="fa <?php if ($_POST['action'] == "add") {
                print "fa-plus";
