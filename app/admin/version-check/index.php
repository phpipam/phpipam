...
# get latest version */
if (!$version = $Tools->check_latest_phpipam_version(true)) {
    $Result->show("danger", _("Version check failed") . '!', false);
} else {
    //print result
    if (VERSION_VISIBLE == $version) {
        $Result->show("success", _('Latest version') . ' (' . VERSION_VISIBLE . ') ' . _('already installed') . '!', false);
    } elseif (VERSION_VISIBLE > $version) {
        $Result->show("success", _('Development version') . ' (' . VERSION_VISIBLE . ') ' . _('installed! Latest production version is') . ' ' . $version, false);
    } else {
        $Result->show("danger", _('New version of phpipam available') . ':</b><hr>' . _('Installed version') . ': ' . VERSION_VISIBLE . "<br>" . _('Available version') . ': ' . $version . "<br><br>" . _('You can download new version') . ' <a href="https://sourceforge.net/projects/phpipam/files/current/phpipam-' . $version . '.tar/download">' . _('here') . '</a>.', false);
    }
}
    ...
    if ($commit_log == "NULL") {
        ...
        foreach ($commit_log as $commit) {
            ...
            foreach ($lines as $k => $l) {
                if (strpos($l, "Author") !== false) {
                    $out['author'] = substr($l, 7);
                    unset($lines[$k]);
                } elseif (strpos($l, "Date") !== false) {
                    $out['date'] = substr($l, 7);
                    unset($lines[$k]);
                } elseif (strpos($l, "Merge:") !== false) {
                    $out['pr'] = $l;
                    unset($lines[$k]);
                } elseif (strlen(trim($l)) == 0) {
                    unset($lines[$k]);
                }
                unset($lines[0]);
            }
