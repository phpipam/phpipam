...
if(sizeof($error) > 0) {
	...
	# execute
	if(!$Admin->object_modify("vaultItems", $_POST['action'], "id", $values)) 	{ $Result->show("danger",  _("Vault item $_POST[action] error"), true); }
	else 																		{ $Result->show("success", _("Vault item $_POST[action] success"), true); }
}