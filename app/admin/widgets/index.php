...
<!-- Add new -->
<button class='btn btn-sm btn-default open_popup' style="margin-bottom:10px;" data-script='app/admin/widgets/edit.php'
        data-class='700' data-action='add'><i class='fa fa-plus'></i> <?php print _('Create new widget'); ?></button>
...
<table class="table sorted table-striped table-top" style="min-width:400px;" data-cookie-id-table="widgets">
    ...
    if ($widgets === false) { ?>
        <tr>
            <td colspan="4">
                <div class="alert alert-info alert-nomargin"><?php print _('No widgets created yet'); ?></div>
            </td>
        </tr>
    <?php } else {
        # headers
        ...
        foreach ($widgets as $w) {
            ...
            if ($valid) {
                $vPrint = "<span class='badge badge1 badge5 alert-success'>" . _('Valid') . "</span>";
            } else {
                $vPrint = "<span class='badge badge1 badge5 alert-danger'>" . _('Invalid') . "</span>";
            }
