...
if($_POST['action']!="add") {
   ...
   @$method_settings->params->hostname = "localhost";
	$method_settings->params->port = 995;
...
<!-- header -->
<div class="pHeader"><?php print _('IMAP connection settings'); ?></div>
...
<div class="pContent">
	...
	<!-- Server -->
	<tr>
		<td style="width:130px;"><?php print _('IMAP server'); ?></td>
		...
		<td class="info2"><?php print _('Enter IMAP server'); ?>
		</td>
	</tr>

<!-- ServerType -->
	<tr>
		<td><?php print _('Server Type'); ?></td>
		<td>
			<select name="servertype" class="form-control input-w-auto">
			<?php
			$values = array(pop3,imap);
			foreach($values as $v) {
				if($v==@$method_settings->params->servertype)	{ print "<option value='$v' selected=selected>$v</option>"; }
	...
	<tr>
		...
		<td class="base_dn info2">
			<?php print _('Enter IMAP port (default 995)'); ?>
		</td>
	</tr>
	...
	if(!in_array("imap", get_loaded_extensions())) {
		$Log->write( "IMAP login", "php-imap extension missing!", 2 );
		$Result->show("danger", _("php-imap extension missing!"), false);
	}
	?>
</div>
