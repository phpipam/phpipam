...
if ($_POST['action'] != 'add' && $_POST['action'] != 'delete') {
    $Result->show("danger", _("Invalid action. Do not manipulate the POST values!") . '<button class="btn btn-sm btn-default hidePopup2">' . _('Cancel') . '</button>', true);
}
# validate $_POST['id'] values
if ($_POST['id'] && !preg_match('/^[0-9]+$/i', $_POST['id'])) {
    $Result->show("danger", _("Invalid ID. Do not manipulate the POST values!") . '<button style="margin-left:50px;" class="btn btn-sm btn-default hidePopup2">' . _('Cancel') . '</button>', true);
}
# validate $_POST['sectionId'] values
if ($_POST['id'] && $_POST['subnetId'] != '') {
    if (!preg_match('/^[0-9]+$/i', $_POST['subnetId'])) {
        $Result->show("danger", _("Invalid subnet ID. Do not manipulate the POST values!") . '<button class="btn btn-sm btn-default hidePopup2">' . _('Cancel') . '</button>', true);
    }
}
...
<div class="pContent">
    ...
    <form id="networkEdit">
                ...
                <tr>
                    ...
                    <td>
                        <select name="sectionId"
                                class="firewallZoneSection form-control input-sm input-w-auto input-max-200">
                            <?php
        ...
        <?php
        if ($_POST['id']) {
            print '<input type="hidden" name="netZoneId" value="' . $_POST['id'] . '">';
        } else {
            print '<input type="hidden" name="noZone" value="1">';
            if ($_POST['network']) {
...
<div class="pFooter">
    <div class="btn-group">
        ...
        <button class="btn btn-sm btn-default <?php if ($_POST['action'] == "delete") {
            ...
            print "btn-success";
        } ?>" id="editNetworkSubmit"><i class="fa <?php if ($_POST['action'] == "add") {
                print "fa-plus";
