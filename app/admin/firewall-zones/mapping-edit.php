    ...
    <form id="mappingEdit">
        ...
        <table class="table table-noborder table-condensed">
            ...
            <tr>
                ...
                <td>
                    <select name="zoneId"
                            class="mappingZoneInformation form-control input-sm input-w-auto input-max-200" <?php print $readonly; ?>>
                        <option value="0"><?php print _('Select a firewall zone'); ?></option>
                        <?php
                        foreach ($firewallZones as $zone) {
                            if ($zone->id == $mapping->id) {
                                if ($zone->description) {
                                    print '<option value="' . $zone->id . '" selected>' . $zone->zone . ' (' . $zone->description . ')</option>';
                                } else {
                                    print '<option value="' . $zone->id . '" selected>' . $zone->zone . '</option>';
                                }
                            } else {
                                if ($zone->description) {
                                    print '<option value="' . $zone->id . '">' . $zone->zone . ' (' . $zone->description . ')</option>';
                                } else {
                                    print '<option value="' . $zone->id . '">' . $zone->zone . '</option>';
                                }
                            }
                        }
                        ?>
            ...
            <tr>
                ...
                <td>
                    <select name="deviceId"
                            class="form-control input-sm input-w-auto input-max-200" <?php print $readonly; ?>>
                            ...
                            foreach ($devices as $device) {
                                if ($device->id == $mapping->deviceId) {
                                    if ($device->description) {
                                        print '<option value="' . $device->id . '" selected>' . $device->hostname . ' (' . $device->description . ')</option>';
                                    } else {
                                        print '<option value="' . $device->id . '" selected>' . $device->hostname . '</option>';
                                    }
                                } else {
                                    if ($device->description) {
                                        print '<option value="' . $device->id . '">' . $device->hostname . ' (' . $device->description . ')</option>';
                                    } else {
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="form-control input-sm" name="interface"
                           placeholder="<?php print _('Firewall interface'); ?>"
                           value="<?php print $mapping->interface; ?>" <?php print $readonly; ?>>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="form-control input-sm" name="alias"
                           placeholder="<?php print _('Local zone alias'); ?>"
                           value="<?php print $mapping->alias; ?>" <?php print $readonly; ?>>
                </td>
            </tr>
        </table>
...
<div class="pFooter">
    <div class="btn-group">
        ...
        <button class="btn btn-sm btn-default <?php if ($_POST['action'] == "delete") {
            ...
            print "btn-success";
        } ?>" id="editMappingSubmit"><i class="fa <?php if ($_POST['action'] == "add") {
                print "fa-plus";
