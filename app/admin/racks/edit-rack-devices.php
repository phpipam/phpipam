...
# device type?
if (!isset($_POST['devicetype']) || (($_POST['devicetype'] != 'device') && ($_POST['devicetype'] != 'content'))) {
    $Result->show("danger", _("Invalid device type"), true, true);
}
    ...
    switch ($_POST['devicetype']) {
        case 'device':
            ...
            if (!$Admin->object_modify("devices", "edit", "id", $values)) {
                ...
                $Result->show("success", _("Device removed from rack") . '!', false, true);
            }
            break;
        
        case 'content':
            if (!$Admin->object_modify('rackContents', 'delete', 'id', ['id' => $_POST['deviceid']])) {
                $Result->show("success", _("Failed to remove device from rack") . '!', true, true);
            } else {
                $Result->show("success", _("Device removed from rack") . '!', false, true);
            }
            ...
            <form id="rackDeviceManagementEdit">
                ...
                <table class="table table-noborder table-condensed">
                        ...
                        <td>
                            ...
                            <?php } else { ?>
                                <input type="text" name="name" class="form-control input-sm"
                                       placeholder="<?php print _('Device name'); ?>">
                            <?php } ?>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="rack_size" class="form-control input-sm"
                                   placeholder="<?php print _('Device size in U'); ?>">
                    ...
                    <?php if ($User->settings->enableLocations == "1" && ($rack->location != "0" && !is_null($rack->location)) && ($_POST['devicetype'] == 'device')) { ?>
                        <tr>
                            <td colspan="2">
                                <hr>
                                <input type="checkbox" class="input-switch" value="1" name="no_location"> <span
                                        class="text-muted"><?php print _("Don't update device location from rack"); ?></span>
                            </td>
                        </tr>
                    <?php } ?>
    ...
    <div class="pFooter">
        <div class="btn-group">
            ...
            <?php if ((!empty($devices)) || ($_POST['devicetype'] != 'device')) { ?>
                <button class="btn btn-sm btn-default btn-success" id="editRackDevicesubmit"><i
                            class="fa fa-plus"></i> <?php print ucwords(_($_POST['action'])); ?></button>
            <?php } ?>
        </div>
