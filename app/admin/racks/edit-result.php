    ...
    if (is_array($rack_devices)) {
        foreach ($rack_devices as $d) {
            ...
            if ($d->rack_start <= $rack_details->size) {
                if (($d->rack_start + $d->rack_size - 1) > $rack['size']) {
                    $Result->show("danger", _('Device') . " $d->hostname " . ("is out of bounds for new rack size" . "!"), true);
                }
            } // back devices
            else {
                if (($d->rack_start - $rack_details->size + $d->rack_size - 1) > $rack['size']) {
                    $Result->show("danger", _('Device') . " $d->hostname " . ("is out of bounds for new rack size" . "!"), true);
                }
            }
        }
...
if ($_POST['action'] == "delete") {
    ...
    try {
        $Database->runQuery("delete from rackContents where `rack` = ?", [$values['id']]);
    } catch (Exception $e) {
    }
}
...
if ($_POST['action'] == "edit" && @$rack['hasBack'] != "1") {
    try {
        $Database->runQuery("update devices set `rack` = 0 where `rack` = ? and rack_start > ?;", [$rack['rackid'], $rack['size']]);
    } catch (Exception $e) {
    }
    try {
        $Database->runQuery("delete from rackContents where `rack` = ? and rack_start > ?;", [$rack['rackid'], $rack['size']]);
    } catch (Exception $e) {
    }
}
...
if ($_POST['action'] == "edit" && @$rack['hasBack'] == "1" && $rack_details->size != $rack['size']) {
    ...
    try {
        $Database->runQuery("UPDATE `devices` set `rack_start` = `rack_start` + :new_size - :old_size where `rack` = :rackid and rack_start > :old_size", $values);
    } catch (Exception $e) {
    }
    try {
        $Database->runQuery("UPDATE `rackContents` set `rack_start` = `rack_start` + :new_size - :old_size where `rack` = :rackid and rack_start > :old_size", $values);
    } catch (Exception $e) {
    }
}
