...
if (($_POST['devicetype'] == 'device') && !is_numeric($_POST['deviceid'])) {
    $Result->show("danger", _("Invalid ID"), true);
}
...
# name
if (($_POST['devicetype'] == 'content') && (!isset($_POST['name']) || (trim($_POST['name'] === '')))) {
    $Result->show("danger", _("Invalid device name"), true);
}
...
# check size
if ($rack->hasBack != "0") {
    if ($_POST['rack_start'] + ($_POST['rack_size'] - 1) > (2 * $rack->size)) {
...
switch ($_POST['devicetype']) {
    ...
    case 'content':
        ...
        # add content
        if (!$Admin->object_modify('rackContents', 'add', null, $values)) {
            $Result->show("success", _("Failed to add device to rack") . '!', false);
        } else {
            $Result->show("success", _("Device added to rack") . '!', false);
        }
