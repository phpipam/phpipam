...
if(isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
	...
	//if cannot move
	else if(!move_uploaded_file($_FILES["file"]["tmp_name"], str_replace("//", "/", $_SERVER['DOCUMENT_ROOT'].BASE."css/custom/logo.png"))) {
		echo '{"status":"error", "error":"Cannot move file to upload dir. You can upload file manually to '.str_replace("//", "/", $_SERVER['DOCUMENT_ROOT'].BASE."css/custom/logo.png").'"}';
		exit;
	}
	else {
        echo '{"status":"success"}';
