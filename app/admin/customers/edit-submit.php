...
// IDs must be numeric
if ($customer['action'] != "add" && !is_numeric($customer['id'])) {
...
if ($customer['action'] != "delete") {
    ...
    if (strlen($_POST['title']) < 3) {
        $Result->show("danger", _("Invalid Title"), true);
    }
    if (strlen($_POST['address']) < 3) {
        $Result->show("danger", _("Invalid Address"), true);
    }
    if (strlen($_POST['city']) < 3) {
        $Result->show("danger", _("Invalid City"), true);
    }
    if (strlen($_POST['state']) < 3) {
        $Result->show("danger", _("Invalid State"), true);
    }
    // check integers
    if (!is_numeric($_POST['postcode'])) {
        $Result->show("danger", _("Invalid Postcode"), true);
    }
}
...
if (sizeof($custom) > 0) {
    foreach ($custom as $myField) {
        ...
        //not null!
        if ($myField['Null'] == "NO" && strlen($customer[$myField['name']]) == 0) {
