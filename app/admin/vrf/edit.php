...
<div class="pContent">
        ...
        <table id="vrfManagementEdit2" class="table table-noborder table-condensed">
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="name form-control input-sm" name="name"
                           placeholder="<?php print _('VRF name'); ?>"
                           value="<?php print $Tools->strip_xss(@$vrf['name']); ?>" <?php print $readonly; ?>>
                </td>
            </tr>
            <!-- RD -->
            <tr>
                ...
                <td>
                    <input type="text" class="rd form-control input-sm" name="rd"
                           placeholder="<?php print _('Route distinguisher'); ?>"
                           value="<?php print $Tools->strip_xss(@$vrf['rd']); ?>" <?php print $readonly; ?>>
                </td>
            </tr>
            <!-- Description -->
            <tr>
                ...
                <td>
                    ...
                    <input type="text" class="description form-control input-sm" name="description"
                           ...
                           value="<?php print $Tools->strip_xss(@$vrf['description']); ?>" <?php print $readonly; ?>>
                </td>
            </tr>
            ...
            if ($User->settings->enableCustomers == 1) {
                ...
                if ($customers != false) {
                    foreach ($customers as $customer) {
                        if ($customer->id == $vrf['customer_id']) {
            ...
            <tr>
                ...
                <td>
                    ...
                    $vrf_sections = is_array($vrf_sections) ? $vrf_sections : [];
                    // loop
                    if ($sections !== false) {
                        foreach ($sections as $section) {
                            if (in_array($section->id, @$vrf_sections)) {
                                print '<div style="margin:0px;"><input type="checkbox" name="section-' . $section->id . '" value="on" checked> ' . $section->name . '</div>' . "\n";
                            } else {
                                print '<div style="margin:0px;"><input type="checkbox" name="section-' . $section->id . '" value="on"> ' . $section->name . '</div>' . "\n";
                            }
                        }
                    }
                    ?>
    ...
    if ($_POST['action'] == "delete") {
        $Result->show("warning", "<strong>" . _('Warning') . ":</strong> " . _("removing VRF will also remove VRF reference from belonging subnets!"), false);
    }
    ?>
</div>
...
<div class="pFooter">
    <div class="btn-group">
        ...
        <button class="btn btn-sm btn-default <?php if ($_POST['action'] == "delete") {
            ...
            print "btn-success";
        } ?>" id="editVRF"><i class="fa <?php if ($_POST['action'] == "add") {
                print "fa-plus";
