...
if ($_POST['action'] != "delete") {
    ...
    if ($_POST['app_security'] == "crypt") {
        if (strlen($_POST['app_code']) != 32 || !ctype_alnum($_POST['app_code'])) {
            $error[] = "Invalid application code";
        }
    }
    # name must be more than 2 and alphanumberic
    if (strlen($_POST['app_id']) < 3 || strlen($_POST['app_id']) > 12 || !ctype_alnum($_POST['app_id'])) {
        $error[] = "Invalid application id";
    }
    # permissions must be 0,1,2
    if ($_POST['app_security'] != "user") {
        if (!($_POST['app_permissions'] == 0 || $_POST['app_permissions'] == 1 || $_POST['app_permissions'] == 2 || $_POST['app_permissions'] == 3)) {
            $error[] = "Invalid permissions";
        }
    }
    # lock check
    if ($_POST['app_lock'] == "1") {
        if (!is_numeric($_POST['app_lock_wait'])) {
            $error[] = "Invalid wait value";
        } elseif ($_POST['app_lock_wait'] < 1) {
            $error[] = "Invalid wait value";
        }
    }
    # api_allow_unsafe check
    if ($_POST['app_security'] == "none" && Config::get('api_allow_unsafe') !== true) {
        $error[] = "API server requires SSL. Please set \$api_allow_unsafe in config.php to override";
    }
}

# default lock_wait
if ($_POST['app_lock_wait'] == "") {
    $_POST['app_lock_wait'] = 0;
}
...
if (sizeof($error) > 0) {
    ...
    # execute
    if (!$Admin->object_modify("api", $_POST['action'], "id", $values)) {
        $Result->show("danger", _("API $_POST[action] error"), true);
    } else {
        $Result->show("success", _("API $_POST[action] success"), true);
    }
}