...
if ($_POST['action'] == "edit" || $_POST['action'] == "add") {
    if (!is_numeric($_POST['local_as'])) {
        $Result->show("danger", _("Invalid local AS"), true);
    }
    if (!is_numeric($_POST['peer_as'])) {
        $Result->show("danger", _("Invalid peer AS"), true);
    }
    if (!$Tools->validate_ip($_POST['local_address'])) {
        $Result->show("danger", _("Invalid local address"), true);
    }
    if (!$Tools->validate_ip($_POST['peer_address'])) {
        $Result->show("danger", _("Invalid peer address"), true);
    }
}
...
if (isset($_POST['vrf_id'])) {
    if ($User->get_module_permissions("vrf") == 0) {
        $Result->show("danger", _("Insufficient permissions for module VRF"), true);
    }
    if (!is_numeric($_POST['vrf_id'])) {
        $Result->show("danger", _("Invalid VRF ID"), true);
    }
}
if (isset($_POST['circuit_id'])) {
    if ($User->get_module_permissions("circuits") == 0) {
        $Result->show("danger", _("Insufficient permissions for module Circuits"), true);
    }
    if (!is_numeric($_POST['circuit_id'])) {
        $Result->show("danger", _("Invalid Circuits ID"), true);
    }
}
if (isset($_POST['customer_id'])) {
    if ($User->get_module_permissions("customers") == 0) {
        $Result->show("danger", _("Insufficient permissions for module Customers"), true);
    }
    if (!is_numeric($_POST['customer_id'])) {
        $Result->show("danger", _("Invalid Customer ID"), true);
    }
}
...
if (!$Admin->object_modify("routing_bgp", $_POST['action'], "id", $values)) {
    $Result->show("danger", _("BGP $_POST[action] failed"), false);
} else {
    $Result->show("success", _("BGP $_POST[action] successful"), false);
}
