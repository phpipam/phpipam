...
foreach ($data as &$cdata) {
    ...
    if (!in_array($cdom, $vdom)) {
        $msg .= "Missing VLAN domain. Please add/import VLAN domain first.";
        $action = "error";
    } else {
    ...
    # check data format
    if ($action != "error") {
        if (!preg_match("/^[a-zA-Z0-9-_]+$/", $cdata['name'])) {
            $msg .= "Invalid name format.";
            $action = "error";
        }
        if (!preg_match("/^[0-9]+$/", $cdata['number'])) {
        ...
        if (!preg_match("/^[a-zA-Z0-9-_. ]+$/", $cdom)) {
            $msg .= "Invalid domain format.";
            $action = "error";
        }
        if ($action != "error") {
            if ($cdata['number'] > $User->settings->vlanMax) {
                $msg .= _('Highest possible VLAN number is ') . $User->settings->vlanMax . '!';
                $action = "error";
            }
        }
    }
    ...
    if (sizeof($custom_fields) > 0) {
        foreach ($custom_fields as $myField) {
            $cfieldtds .= "<td>" . $cdata[$myField['name']] . "</td>";
        }
    }
    
    # check if duplicate VLAN
    if (isset($unique[$cdom][$cdata['number']])) {
        $msg .= "Duplicate VLAN domain and number not supported. Please check import file.";
        $action = "error";
    }
    ...
    if ($action != "error") {
        if (isset($edata[$cdom][$cdata['number']])) {
            ...
            if ($cdata['name'] != $edata[$cdom][$cdata['number']]['name']) {
                $msg .= "VLAN name will be updated.";
                $action = "edit";
            }
            if ($cdata['description'] != $edata[$cdom][$cdata['number']]['description']) {
                $msg .= "VLAN description will be updated.";
                $action = "edit";
            }
            ...
            if (sizeof($custom_fields) > 0) {
                foreach ($custom_fields as $myField) {
                    if ($cdata[$myField['name']] != $edata[$cdom][$cdata['number']][$myField['name']]) {
                        $msg .= "VLAN " . $myField['name'] . " will be updated.";
                        $action = "edit";
                    }
                }
            }
    ...
    $counters[$action]++;
    if (!isset($unique[$cdom][$cdata['number']])) {
        $unique[$cdom][$cdata['number']] = $cdata['name'];
    }
