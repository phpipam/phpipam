...
foreach ($data as &$cdata) {
    ...
    if ($action != "error") {
        if (preg_match("/\//", $cdata['subnet'])) {
            ...
            if ((!isset($cdata['mask'])) or ($cdata['mask'] == "")) {
                $msg .= "Required field mask missing or empty.";
                $action = "error";
            }
        }
    ...
    if (!isset($vlan_data[$cdom])) {
        ...
        $msg .= "Invalid VLAN domain.";
        $action = "error";
    } else {
        if (!empty($cdata['vlan'])) {
            if (in_array(strtolower($cdata['vlan']), ["na", "n/a", "nan"])) {
                $cdata['vlan'] = "";
            }
    ...
    if ($action != "error") {
        ...
        if ((!empty($cdata['vrf'])) && (!preg_match("/^[a-zA-Z0-9-_]+$/", $cdata['vrf']))) {
...
#		if ((!empty($cdata['vlan'])) && (!preg_match("/^[0-9]+$/", $cdata['vlan']))) { $msg.="Invalid VLAN number format."; $action = "error"; }
        if ((!empty($cdata['domain'])) && (!preg_match("/^[a-zA-Z0-9-_. ]+$/", $cdata['domain']))) {
            $msg .= "Invalid VLAN domain format.";
            $action = "error";
        }
    }
    ...
    if ($action != "error") {
        if (isset($ndata[$cdata['sectionId']][$cdata['vrfId']][$cdata['subnet']][$cdata['mask']])) {
    ...
    if ($action != "error") {
        if (isset($edata[$cdata['sectionId']][$cdata['vrfId']][$cdata['subnet']][$cdata['mask']])) {
            ...
            if ($cdata['description'] != $cedata['description']) {
                $msg .= "Subnet description will be updated.";
                $action = "edit";
            }
            if ($cdata['vlanId'] != $cedata['vlanId']) {
                $msg .= "VLAN ID will be updated.";
                $action = "edit";
            }
