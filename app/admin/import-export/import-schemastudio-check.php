...
$schema = $Tools->fetch_all_objects("schemastudios", "id");
$studiotypes = $Tools->fetch_all_objects("studiotypes", "id");
...
foreach ($schema as $d) {
    $d = (array) $d;
    $edata['schema'][strtolower($d['studioType'])][strtolower($d['description'])] = $d;
}

foreach ($studiotypes as $d) {
    $d = (array) $d;
    $edata['studiotypes'][strtolower($d['studioType'])] = $d;
}
...
foreach ($data as &$cdata) {
	...
	if (!isset($edata['studiotypes'][strtolower($cdata['studioType'])])
	    ) {
		$msg.= "Invalid Studio Type."; $action = "error";
	} else {
		$cdata['studioType'] = $edata['studiotypes'][strtolower($cdata['studioType'])]['id'];
	}
	...
	if ($action != "error") {
		if (isset($edata['schema'][strtolower($cdata['studioType'])][strtolower($cdata['description'])]) ) {
    		$cdata['id'] = $edata['schema'][strtolower($cdata['studioType'])][strtolower($cdata['description'])]['id'];
			# copy content to a variable for easier checks
			$cedata = $edata['schema'][strtolower($cdata['studioType'])][strtolower($cdata['description'])];
			if (strtolower($cdata['parent'])=="none"){$cdata['parent']=0;}
			else {
				$schemas=$Tools->fetch_multiple_objects("schemastudios","studioType",$cdata['studioType']);
