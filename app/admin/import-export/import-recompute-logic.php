...
foreach ($all_sections as $section) {
    ...
    $sect_names[$section['id']] = $section['name'];
}
...
foreach ($rlist as $sect_id => $sect_check) {
    ...
    foreach ($section_subnets as &$c_subnet) {
        if (!$sect_check[$c_subnet['type']]) {
            continue;
        }    # Skip the IP version we don't want to reorder
        if ($c_subnet['isFolder']) {
            continue;
        } # Skip folders
        if ($subnetbyid[$c_subnet['masterSubnetId']]['isFolder']) {
            continue;
        } # Skip changing subnet with folder masters
        ...
        while (--$search_mask >= 0) {
            ...
            if (!$sect_check["CVRF"]) {
                foreach ($t_candidate as $i => $t_subnet) {
                    if ($t_subnet['vrfId'] != $c_subnet['vrfId']) {
                        unset($t_candidate[$i]);
                    }
                }
        ...
        foreach ($m_candidate as $m_subnet) {
            ...
            if ($m_subnet['vrfId'] == $c_subnet['vrfId']) {
                $c_master_id = $m_subnet['id'];
                $c_master_mask = $m_subnet['mask'];
                $c_master_ip = $m_subnet['ip'];
                $search_child_vrf_only = 1;
            }
            
            # Previous candidate found in child VRF. Ignore candidates from other VRFs
            if ($search_child_vrf_only == 1 && $m_subnet['vrfId'] != $c_subnet['vrfId']) {
                continue;
            }
        ...
        if ($_GET['recomputeHideUnchanged'] == "on" && $c_subnet['masterSubnetId'] == $c_master_id) {
            continue;
        }
