...
foreach ($data as &$cdata) {
    $used_section[strtolower($cdata['section'])] = $cdata['section'];
}
...
foreach ($all_sections as $section) {
    ...
    if (!isset($used_section[strtolower($section['name'])])) {
        continue;
    }
...
if ($devices !== false) {
    foreach ($devices as $c_dev) {
        ...
        foreach ($c_dev_sections as $c_dev_sect) {
            $device_data[$c_dev_sect][$c_dev['hostname']] = $c_dev;
        }
    }
}
...
foreach ($data as &$cdata) {
    ...
    foreach ($reqfields as $creq) {
        if ((!isset($cdata[$creq]) or ($cdata[$creq] == ""))) {
            $msg .= "Required field " . $creq . " missing or empty.";
            $action = "error";
        }
    }
    ...
    if ($action != "error") {
        if (preg_match("/\//", $cdata['subnet'])) {
            ...
            $msg .= "The subnet needs to have the mask defined as /BM (Bit Mask)";
            $action = "error";
        }
        if ((!empty($cdata['mask'])) && (!preg_match("/^([0-9]+|[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)$/", $cdata['mask']))) {
            $msg .= "Invalid network mask format.";
            $action = "error";
        } else {
            $cdata['type'] = $Subnets->identify_address($cdata['subnet']);
            if (($cdata['type'] == "IPv6") && (($cdata['mask'] < 0) || ($cdata['mask'] > 128))) {
                $msg .= "Invalid IPv6 network mask.";
                $action = "error";
            }
        }
    }
        ...
        if (!isset($subnet_data[$cdata['sectionId']][$cdata['vrfId']][$cdata['subnet']][$cdata['mask']])) {
            ...
            if (isset($subnet_search[$cdata['sectionId']][$cdata['subnet']][$cdata['mask']])) {
                ...
                if (sizeof($results) == 1) {
                    ...
                    $msg .= "Search matches multiple subnets, please specify VRF.";
                    $action = "error";
                }
            }
        }
    ...
    if ($action != "error") {
        if (!isset($subnet_data[$cdata['sectionId']][$cdata['vrfId']][$cdata['subnet']][$cdata['mask']])) {
            $msg .= "Unable to locate the subnet in the specified VRF.";
            $action = "error";
        } else {
    ...
    # Verify gateway
    if (in_array(strtolower($cdata['is_gateway']), ["yes", "true", "1"])) {
        $cdata['is_gateway'] = 1;
    } else {
        $cdata['is_gateway'] = 0;
    }
    ...
    if ($action != "error") {
        ...
        if ((!empty($cdata['hostname'])) and (!preg_match("/^(?=.{1,255}$)[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?(?:\.[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?)*\.?$/", $cdata['hostname']))) {
        ...
        if (preg_match("/[;'\"]/", $cdata['owner'])) {
            $msg .= "Invalid characters in owner name.";
            $action = "error";
        }
        if (preg_match("/[;'\"]/", $cdata['note'])) {
            $msg .= "Invalid characters in note.";
            $action = "error";
        }
    }
    ...
    if ($action != "error") {
        ...
        if (!$Subnets->verify_overlapping($ip_cidr, $subnet_cidr)) {
            $msg .= "Invalid IP address, not inside subnet.";
            $action = "error";
        }
    }
    ...
    if ($action != "error") {
        if (isset($ndata[$cdata['sectionId']][$cdata['vrfId']][$cdata['subnet']][$cdata['mask']][$cdata['ip_addr']])) {
            $msg .= "Duplicate entry in imported data.";
            $action = "error";
        }
    }
    ...
    if ($action != "error") {
        if (isset($edata[$cdata['sectionId']][$cdata['vrfId']][$cdata['subnet']][$cdata['mask']][$cdata['ip_addr']])) {
            ...
            if ($cdata['hostname'] != $cedata['hostname']) {
                $msg .= "Address DNS name will be updated.";
                $action = "edit";
            }
            if ($cdata['description'] != $cedata['description']) {
                $msg .= "Address description will be updated.";
                $action = "edit";
            }
            if ($cdata['mac'] != $cedata['mac']) {
                $msg .= "Address MAC address will be updated.";
                $action = "edit";
            }
            if ($cdata['owner'] != $cedata['owner']) {
                $msg .= "Address owner will be updated.";
                $action = "edit";
            }
            if ($cdata['switch'] != $cedata['switch']) {
                $msg .= "Device will be updated.";
                $action = "edit";
            }
            if ($cdata['note'] != $cedata['note']) {
                $msg .= "Address note will be updated.";
                $action = "edit";
            }
            if ($cdata['state'] != $cedata['state']) {
                $msg .= "Address tag (state) will be updated.";
                $action = "edit";
            }
            if ($cdata['port'] != $cedata['port']) {
                $msg .= "Port will be updated.";
                $action = "edit";
            }
            if ($cdata['location'] != $cedata['location']) {
                $msg .= "Location will be updated.";
                $action = "edit";
            }
            ...
            if (sizeof($custom_fields) > 0) {
                foreach ($custom_fields as $myField) {
                    if ($cdata[$myField['name']] != $cedata[$myField['name']]) {
                        $msg .= $myField['name'] . " will be updated.";
                        $action = "edit";
                    }
                }
            }
