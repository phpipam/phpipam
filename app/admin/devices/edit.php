...
<script type="text/javascript">
    ...
    $('#switchManagementEdit select[name=rack]').change(function () {
        ...
        // load dropdown
        $.post("app/admin/devices/edit-rack-dropdown.php", {
            rackid: $('#switchManagementEdit select[name=rack]').val(),
            deviceid: $('#switchManagementEdit input[name=switchid]').val(),
            action: $('#switchManagementEdit input[name=action]').val()
        }, function (data) {
    ...
    <form id="switchManagementEdit">
        <table class="table table-noborder table-condensed">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="hostname" class="form-control input-sm"
                           placeholder="<?php print _('Hostname'); ?>"
                           value="<?php if (isset($device['hostname'])) print $Tools->strip_xss($device['hostname']); ?>" <?php print $readonly; ?>>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="ip_addr" class="form-control input-sm"
                           placeholder="<?php print _('IP address'); ?>"
                           value="<?php if (isset($device['ip_addr'])) print $Tools->strip_xss($device['ip_addr']); ?>" <?php print $readonly; ?>>
                </td>
            </tr>
                ...
                <td>
                    <select name="type" class="form-control input-sm input-w-auto">
                        ...
                        foreach ($types as $name) {
                            if ($device['type'] == $name->tid) {
                                print "<option value='$name->tid' selected='selected'>$name->tname</option>";
                            } else {
                                print "<option value='$name->tid' >$name->tname</option>";
                            }
                        }
                        ?>
                    </select>
                        ...
                        <select name="rack" class="form-control input-sm">
                            ...
                            if ($Racks->all_racks !== false) {
                                foreach ($Racks->all_racks as $r) {
                                    if ($device['rack'] == $r->id) {
                                        print "<option value='$r->id' data-location='$r->location' selected>$r->name</option>";
                                    } else {
                                        print "<option value='$r->id' data-location='$r->location'>$r->name</option>";
                                    }
                                }
                            }
                            ?>
            ...
            <tr>
                ...
                <td>
                    <textarea name="description" class="form-control input-sm"
                              placeholder="<?php print _('Description'); ?>" <?php print $readonly; ?>><?php if (isset($device['description'])) print $device['description']; ?></textarea>
                    <?php
                    if (($_POST['action'] == "edit") || ($_POST['action'] == "delete")) {
                    ...
                    if ($sections !== false) {
                        foreach ($sections as $section) {
                            if (in_array($section->id, $deviceSections)) {
                                print '<div class="checkbox" style="margin:0px;"><input type="checkbox" name="section-' . $section->id . '" value="on" checked> ' . $section->name . '</div>' . "\n";
                            } else {
                                print '<div class="checkbox" style="margin:0px;"><input type="checkbox" name="section-' . $section->id . '" value="on">' . $section->name . '</span></div>' . "\n";
                            }
                        }
                    }
                    ?>
...
<div class="pFooter">
    <div class="btn-group">
        ...
        <button class="btn btn-sm btn-default <?php if ($_POST['action'] == "delete") {
            print "btn-danger";
        } else {
            print "btn-success";
        } ?>" id="editSwitchsubmit"><i class="fa <?php if ($_POST['action'] == "add") {
                print "fa-plus";
