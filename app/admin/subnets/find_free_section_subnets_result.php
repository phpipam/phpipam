...
if (!is_numeric($_POST['mask']) || !is_numeric($_POST['results'])) {
    $Result->show("danger", _('Invalid parameters') . "!", true);
}

// mask check
if ($_POST['version'] == "IPv4" && ($_POST['mask'] < 8 || $_POST['mask'] > 32)) {
    $Result->show("danger", _('Invalid mask') . "!", true);
}
if ($_POST['version'] == "IPv6" && ($_POST['mask'] < 8 || $_POST['mask'] > 128)) {
    $Result->show("danger", _('Invalid mask') . "!", true);
}
...
if (!is_numeric($_POST['subnet_start'])
    ...
    || $_POST['subnet_start'] == $_POST['subnet_end']) {
    $Result->show("danger", _('Invalid range') . "!", true);
}

// version check
if ($Subnets->identify_address($_POST['subnet_start']) !== $Subnets->identify_address($_POST['subnet_end'])) {
    $Result->show("danger", _('Invalid range') . "!", true);
}
...
// no subnets
if (sizeof($all_subnets) == 0) {
    $Result->show("warning", _('No subnets found') . "!", true);
} // single
elseif (sizeof($all_subnets) == 1) {
    $Result->show("warning", _('Only one subnet found') . "!", true);
} // process
else {
    // set start and stop
