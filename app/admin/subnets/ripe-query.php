            ...
            foreach ($res['data'] as $k => $d) {
                ...
                if (is_array($custom_fields)) {
                    foreach ($custom_fields as $f) {
                        ...
                        if (strtolower($f['name']) == strtolower($k)) {
                            print "<option values='$f[name]' selected='selected'>$f[name]</option>";
                        } else {
                            print "<option values='$f[name]'>$f[name]</option>";
                        }
                    }
                }
                print "</select>";
...
<div class="pFooter">
    <div class="btn-group">
        ...
        <?php if ($res['result'] != "error") { ?>
            <button class="btn btn-sm btn-default btn-success" id="ripeMatchSubmit"><i
                        class="fa fa-check"></i> <?php print _('fill'); ?></button>
        <?php } ?>
    </div>
</div>