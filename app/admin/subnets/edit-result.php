...
$errors = [];
# default vrf
if (!isset($_POST['vrfId']) || $_POST['vrfId'] == null) {
    $_POST['vrfId'] = 0;
}
if (!is_numeric($_POST['vrfId'])) {
    $_POST['vrfId'] = 0;
}
...
elseif ($_POST['action'] == "delete" && !isset($_POST['deleteconfirm'])) {
    ...
    if (!isset($_POST['pingSubnet']) && !isset($_POST['discoverSubnet']) && !isset($_POST['resolveDNS'])) {
        $_POST['scanAgent'] = 0;
    }
        ...
        if ((isset($values['sectionId']) && $_POST['action'] == "edit")) {
            ...
            if (sizeof($Subnets->slaves) > 0) {
                foreach ($Subnets->slaves as $slaveId) {
                    $Admin->object_modify("subnets", "edit", "id", ["id" => $slaveId, "sectionId" => $_POST['sectionIdNew']]);
                }
            }
        }
    ...
    if ($User->settings->enablePowerDNS == 1) {
        ...
        if (!isset($_POST['DNSrecursive']) && @$old_subnet_details->DNSrecursive == 0) {
