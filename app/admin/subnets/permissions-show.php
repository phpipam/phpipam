...
<div class="pContent">
    ...
    if ($subnet->isFolder == 1) {
        print _('Manage permissions for folder') . " $subnet->description";
    } else {
        print _('Manage permissions for subnet'); ?><?php print $Subnets->transform_to_dotted($subnet->subnet) . "/" . $subnet->mask . " ($subnet->description)";
    }
    ?>
    <hr>
    ...
    <form id="editSubnetPermissions">
        <table class="editSubnetPermissions table table-noborder table-condensed">
            ...
            if (strlen($subnet->permissions) > 1) {
                $permissons = $Sections->parse_section_permissions($subnet->permissions);
            } else {
                $permissons = "";
            }
            ...
            <?php if ($Subnets->has_slaves($_POST['subnetId'])) { ?>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="checkbox" style="margin-left: 0px; padding-left: 0px;" name="set_inheritance"
                               class="input-switch" value="Yes" <?php print $checked; ?>>
                    </td>
                </tr>
                <tr class="warning2">
                    <td colspan="2">
...
<div class="pFooter">
    <div class="btn-group">
        ...
        <button class="btn btn-sm btn-default btn-success editSubnetPermissionsSubmit"><i
                    class="fa fa-check"></i> <?php print _('Set permissions'); ?></button>
    </div>
