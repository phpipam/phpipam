...
<!-- Add new -->
<button class='btn btn-sm btn-default open_popup' style="margin-bottom:10px;" data-script='app/admin/languages/edit.php'
        data-class='700' data-action='add'><i class='fa fa-plus'></i> <?php print _('Create new language'); ?></button>


<table class="table sorted nosearch nopagination table-striped table-auto table-top" data-cookie-id-table="admin_lang"
    ...
    if ($languages == false) { ?>
        <tr>
            <td colspan="4">
                <div class="alert alert-info alert-nomargin"><?php print _('No languages present created yet'); ?></div>
            </td>
        </tr>
    <?php } else {
        # headers
        ...
        foreach ($languages as $lang) {
            ...
            # set valid text
            if ($valid) {
                $vPrint = "<span class='alert alert-success btn-sm'>" . _('Valid') . "</span>";
            } else {
                $vPrint = "<span class='alert alert-danger  btn-sm'>" . _('Invalid') . "</span>";
            }
