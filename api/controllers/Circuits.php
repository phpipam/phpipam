...
class Circuits_controller extends Common_api_functions
{
    ...
    private function rewrite_controller_params($params)
    {
        ...
        unset($params['id']);
        if (isset($params['id2'])) {
            $this->_params->id = $params['id2'];
            unset($params['id2']);
        }
        if (isset($params['id3'])) {
            $this->_params->id2 = $params['id3'];
            unset($params['id3']);
        }
        if (isset($params['id4'])) {
            $this->_params->id3 = $params['id4'];
            unset($params['id4']);
        }
        // post, get etc
        if (sizeof($params) > 0) {
    ...
    public function GET()
    {
        ...
        if (!isset($this->_params->id) || $this->_params->id == "all") {
            ...
            // check result
            if ($result === false) {
                $this->Response->throw_exception(200, "No {$this->type_text}s configured");
            } else {
                return ["code" => 200, "data" => $this->prepare_result($result, null, true, true)];
            }
        } // provider circuits
        elseif ($this->type === "circuitProviders" && isset($this->_params->id2)) {
            if ($this->_params->id2 == "circuits") {
                ...
                // check result
                if ($result == null) {
                    $this->Response->throw_exception(200, "No circuits belonging to provider");
                } else {
                    return ["code" => 200, "data" => $this->prepare_result($result, null, true, true)];
                }
            } else {
        ...
        elseif ($this->type === "circuits" && ($this->_params->id == "circuit_id" || $this->_params->id == "id")) {
            ...
            // check result
            if ($result == null) {
                $this->Response->throw_exception(200, "{$this->type_text} not found");
            } else {
                return ["code" => 200, "data" => $this->prepare_result($result, null, true, true)];
            }
        } // read details
    ...
    private function validate_id($action = "add")
    {
        ...
        if ($action !== "add") {
            ...
            if (!isset($this->_params->id)) {
                $this->Response->throw_exception(400, "{$this->type_text} id is required");
            }
            // validate number
            if (!is_numeric($this->_params->id)) {
                $this->Response->throw_exception(400, "{$this->type_text} id must be numeric");
            }
        }
        // for delete
        if ($action == "delete" || $action == "edit") {
            if ($this->Tools->fetch_object($this->type, "id", $this->_params->id) === false) {
                $this->Response->throw_exception(404, "Nonexisting {$this->type_text} id");
            }
        }
    }
        ...
        if ($old_object->cid !== null) {
            if (isset($this->_params->cid)) {
                if ($old_object->cid != $this->_params->cid) {
                    if ($this->Tools->fetch_object($this->type, "cid", $this->_params->cid)) {
                        $this->Response->throw_exception(409, "Circuit ID already exists");
                    }
                }
            }
        } // add
            ...
            // mandatory
            if (!isset($this->_params->cid)) {
                $this->Response->throw_exception(400, "Circuit ID is mandatory");
            } // check
            elseif ($this->Tools->fetch_object($this->type, "cid", $this->_params->cid)) {
                $this->Response->throw_exception(409, "Circuit ID already exists");
            }
        }
    }
    ...
    {
        if ($action == "add") {
            if ($this->Tools->fetch_object("circuitProviders", "id", $this->_params->provider) === false) {
                $this->Response->throw_exception(400, "Invalid circuit provider");
            }
    ...
    private function validate_circuit_type($action = "add")
    {
        if (isset($this->_params->type)) {
            ...
            $all_types = explode(",", str_replace(["enum", "(", ")", "'"], "", $type_desc->Type));
            if (!in_array($this->_params->type, $all_types)) {
                $this->Response->throw_exception(400, "Invalid circuit type");
            }
        } else {
            if ($action == "add") {
    ...
    private function validate_circuit_status($action = "add")
    {
        if (isset($this->_params->status)) {
            ...
            if (!in_array($this->_params->status, $statuses)) {
                $this->Response->throw_exception(400, "Invalid status");
            }
        } else {
            if ($action == "add") {
        ...
        for ($m = 1; $m < 3; $m++) {
            if (!isset($this->_params->{device . $m}) && !isset($this->_params->{location . $m})) {
                ...
                if ($this->_params->{device . $m} !== null) {
                    if ($this->Tools->fetch_object("devices", "id", $this->_params->{device . $m}) === false) {
                        $this->Response->throw_exception(400, "Invalid device $m");
                    }
                    $this->_params->{location . $m} = 0;
                } else {
                    if ($this->Tools->fetch_object("locations", "id", $this->_params->{location . $m}) === false) {
                        $this->Response->throw_exception(400, "Invalid location $m");
                    }
                    $this->_params->{device . $m} = 0;
                }
            }
    ...
    private function validate_provider_name($action)
    {
        if ($action == "add") {
            if (!isset($this->_params->name)) {
                $this->Response->throw_exception(404, "Name is mandatory");
    ...
    {
        if (sizeof($values) == 0) {
            $this->Response->throw_exception(409, "No values present");
        }
    }
