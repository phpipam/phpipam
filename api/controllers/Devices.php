...
class Devices_controller extends Common_api_functions
{
    ...
    public function GET()
    {
        ...
        if (!isset($this->_params->id) || $this->_params->id == "all") {
            ...
            // result
            if (!$result) {
                return $this->Response->throw_exception(200, "No devices configured");
            } else {
            ...
            if ($this->_params->id == 'search') {
                ...
                if (isset($this->_params->id2)) {
                    ...
                    // result
                    if (!$result) {
                        return $this->Response->throw_exception(200, "No devices found");
                    } else {
                        return ['code' => 200, 'data' => $this->prepare_result($result, 'devices', true, false)];
                    }
                } else {
                ...
                if (!is_numeric($this->_params->id)) {
                    $this->Response->throw_exception(400, 'ID must be numeric');
                }
                ...
                if ($result === false) {
                    return $this->Response->throw_exception(200, "No " . $this->_params->id2 . " found");
                } else {
                    return ['code' => 200, 'data' => $this->prepare_result($result, 'devices', true, false)];
                }
            }
