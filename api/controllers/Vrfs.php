...
class Vrfs_controller extends Common_api_functions
{
    ...
    public function GET()
    {
        ...
        if (!isset($this->_params->id) || $this->_params->id == "all") {
            ...
            // check result
            if ($result === false) {
                $this->Response->throw_exception(200, 'No vrfs configured');
            } else {
        ...
        elseif (isset($this->_params->id2)) {
            ...
            if ($this->_params->id2 == "subnets") {
                ...
                // check result
                if ($result === false) {
                    $this->Response->throw_exception(200, 'No subnets belonging to this vrf');
                } else {
    ...
    private function validate_vrf()
    {
        ...
        if (!isset($this->_params->id)) {
            $this->Response->throw_exception(400, "Vrf Id is required");
        }
        // validate number
        if (!is_numeric($this->_params->id)) {
            $this->Response->throw_exception(400, "Vrf Id must be numeric");
        }
        // check that it exists
        if ($this->Tools->fetch_object("vrf", "vrfId", $this->_params->id) === false) {
            $this->Response->throw_exception(400, "Invalid VRF id");
        }
    }
    ...
    private function validate_vrf_edit()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if (strlen($this->_params->name) == 0) {
                $this->Response->throw_exception(400, "VRF name is required");
            }
            // check that it exists
