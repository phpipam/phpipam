...
$("input[name='subnet']").change(function() {
	...
	$masterdopdown.load('<?php print 'app/subnets/mastersubnet-dropdown.php?section='.urlencode((string) $POST->sectionId).'&cidr='; ?>' + $(this).val() + '&prev=' + $masterdopdown.val());
});

<?php if($POST->location=="ipcalc" && !isset($POST->freespaceMSID)) { ?>
    var $masterdopdown = $("select[name='masterSubnetId']");
    $masterdopdown.load('<?php print 'app/subnets/mastersubnet-dropdown.php?section='.urlencode((string) $POST->sectionId).'&cidr='; ?>' + $(this).val() + '&prev=0');
<?php } ?>
    ...
    <tr>
        ...
        <td class="info2">
            <div class="btn-group">
            	...
            	<?php if($User->settings->enableSNMP == "1" && $POST->action=="add") { $csrf_scan = $User->Crypto->csrf_session_token(); ?>
