...
if(sizeof($all_sections) > 0) {
	...
	foreach ($sections_sorted as $section) {
		...
		$section_rows.= "<tr>";
		$section_rows.= "<td><div class='checkbox'><label><input type='checkbox' id='recomputeSectionCheck' name='recomputeSection_".$section['id']."' checked>".$sect_prefix.str_replace('_', ' ', (string) $section['name'])."</label></div></td>";
