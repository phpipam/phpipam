				...
				foreach ($loop['values'] as $i) {
					if (isset($all_contents[$i])) {
						foreach ($all_contents[$i] as $cur) {
							...
							if ($User->get_module_permissions ("racks")>=User::ACCESS_RW) {
								print "<a href='' class='btn btn-xs btn-default btn-danger editRackDevice' data-action='remove' rel='tooltip' data-html='true' data-placement='left' title='"._("Remove")."' data-action='remove' style='margin-bottom:2px;margin-right:5px;' data-rackid='$rack->id' data-deviceid='$cur->id' data-devicetype='{$itemType}' data-csrf='".$User->Crypto->csrf_session_token()."'><i class='fa fa-times'></i></a> ";
							}
 ...
 * keeping this code here on purpose for comparison in case issues pop up in the next week 2025-03-23
            ...
            do {
                ...
                if($User->get_module_permissions ("racks")>=User::ACCESS_RW) {
                    print "<a href='' class='btn btn-xs btn-default btn-danger editRackDevice' data-action='remove' rel='tooltip' data-html='true' data-placement='left' title='"._("Remove")."' data-action='remove' style='margin-bottom:2px;margin-right:5px;' data-rackid='$rack->id' data-deviceid='$cur->id' data-devicetype='$ctype' data-csrf='".$User->Crypto->csrf_session_token()."'><i class='fa fa-times'></i></a> ";
                }
                print "<span class='badge badge1 badge5 $error' style='margin-bottom:3px;margin-right:5px;'>"._("Position").": $cur->rack_start_print, "._("Size").": $cur->rack_size U</span>";
                if ($ctype == 'device') {
                    print " <a href='".create_link("tools", "devices", $cur->id)."'>$cur->hostname</a><br>";
