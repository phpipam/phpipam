			...
			if(sizeof($addresses) == 0) {
    			...
    			foreach ($addresses as $address) {
					...
					if(in_array('mac', $selected_ip_fields)) {
    					...
    					// multicast ?
    					if ($User->settings->enableMulticast=="1" && $Subnets->is_multicast ($address->ip_addr))          { print "<td class='$mclass' style='white-space:nowrap;'>".$address->mac." $minfo $mobjects_txt</td>"; }
