...
class User extends Common_functions
{
    ...
    public function block_check_ip()
    {
        ...
        # fetch
        try {
            $cnt = $this->Database->getObjectQuery($query, [$this->ip, $now]);
        } catch (Exception $e) {
    ...
    private function purge_blocked_entries()
    {
        ...
        try {
            $this->Database->runQuery($query, [$ago]);
        } catch (Exception $e) {
            !$this->debugging ?: $this->Result->show("danger", $e->getMessage(), false);
        }
    }
    ...
    public function update_activity_time()
    {
        # update
        try {
            $this->Database->updateObject("users", ["lastActivity" => date("Y-m-d H:i:s"), "id" => $this->user->id]);
        } catch (Exception $e) {
        }
    }
    ...
    public function set_maintaneance_mode($on = false)
    {
        ...
        # execute
        try {
            $this->Database->updateObject("settings", ["id" => 1, "maintaneanceMode" => $maintaneance_mode], "id");
        } catch (Exception $e) {
        }
    }
    ...
    private function remove_favourite($subnetId)
    {
        ...
        # update
        try {
            $this->Database->updateObject("users", ["favourite_subnets" => $new_favourites, "id" => $this->user->id], "id");
        } catch (Exception $e) {
            return false;
        }
        return true;
        ...
        if (strlen($this->user->favourite_subnets) == 0) {
            ...
            if (sizeof($subnets) > 0) {
                ...
                foreach ($subnets as $id) {
                    ...
                    try {
                        $fsubnet = $this->Database->getObjectQuery($query, [$id]);
                    } catch (Exception $e) {
    ...
    {
        try {
            $this->Database->updateObject("users", ["password" => $this->crypt_user_pass($password), "passChange" => "No", "id" => $this->user->id], "id");
        } catch (Exception $e) {
    ...
    public function self_update($post)
    {
        ...
        # set items to update
        $items = ["real_name" => escape_input(strip_tags($post['real_name'])),
    ...
    public function self_update_widgets($widgets)
    {
        # update
        try {
            $this->Database->updateObject("users", ["widgets" => $widgets, "id" => $this->user->id]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
    ...
    private function auth_local($username, $password)
    {
        ...
        if (hash_equals($this->user->password, crypt($password, $this->user->password))) {
            ...
            # apache
            if (!empty($_SERVER['PHP_AUTH_USER']) && $this->api !== true) {
    ...
    public function update_login_time()
    {
        ...
        if ($this->settings->version != "1.1") {
            # update
            try {
                $this->Database->updateObject("users", ["lastLogin" => date("Y-m-d H:i:s"), "id" => $this->user->id]);
            } catch (Exception $e) {
                $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
                return false;
            }
