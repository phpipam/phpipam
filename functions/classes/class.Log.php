...
class Logging extends Common_functions
{
    ...
    public function write($command, $details = null, $severity = 0, $username = null)
    {
        ...
        if ($this->log_type == "syslog") {
            $this->syslog_write();
        } elseif ($this->log_type == "both") {
            $this->database_write_log();
            $this->syslog_write();
        } else {
            $this->database_write_log();
        }
    ...
    private function syslog_set_facility()
    {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            ...
            if (strpos($this->log_command, "login") > 0 ||
                strpos($this->log_command, "logged out") > 0) {
                $facility = "LOG_AUTH";
            } else {
                $facility = "LOG_USER";
            }
    ...
    private function syslog_set_priority()
    {
        ...
        # set
        if ($this->log_severity == "2") {
            $priority = "LOG_ALERT";
        } elseif ($this->log_severity == "1") {
    ...
    private function database_write_log()
    {
        ...
        # execute
        try {
            $this->Database->insertObject("logs", $values);
        } catch (Exception $e) {
    ...
    public function fetch_logs($logCount, $direction = null, $lastId = null, $highestId = null, $informational = "off", $notice = "off", $warning = "off")
    {
        ...
        if (!is_numeric($logCount)) {
            $this->Result->show("danger", "Invalid logcount value", true);
            return false;
        }
        if ($direction !== null) {
            if ($direction != "next" && $direction != "prev" && $direction != "") {
                {
                    $this->Result->show("danger", "Invalid direction", true);
                    return false;
                }
            }
        }
        ...
        # fetch
        try {
            $logs = $this->Database->getObjectsQuery(implode("\n", $query), $params);
        } catch (Exception $e) {
    ...
    public function log_fetch_highest_id()
    {
        # fetch
        try {
            $id = $this->Database->getObjectQuery("select id from logs order by id desc limit 1;");
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), false);
            return false;
        }
    ...
    public function write_changelog($object_type, $action, $result, $old = [], $new = [], $mail_changelog = true)
    {
        ...
        if ($this->settings->enableChangelog == 1) {
            ...
            if (is_array($log) && sizeof($log) > 0) {
                ...
                // execute
                if ($this->log_type == "syslog") {
                    $this->syslog_write_changelog($log);
                } elseif ($this->log_type == "both") {
                    $this->changelog_write_to_db($log);
                    $this->syslog_write_changelog($log);
                } else {
                    $this->changelog_write_to_db($log);
                }
    ...
    private function changelog_unset_unneeded_values()
    {
        ...
        elseif ($this->object_type == "subnet" || $this->object_type == "folder") {
            ...
            if (strlen($this->object_new['subnet']) > 0) {
                $this->object_new['subnet'] = $this->Subnets->transform_address($this->object_new['subnet'], "dotted");
            }
            if (strlen($this->object_old['subnet']) > 0) {
                $this->object_old['subnet'] = $this->Subnets->transform_address($this->object_old['subnet'], "dotted");
            }
            
            //remove subnet/mask for folders
            if (@$this->object_new['isFolder'] == "1") {
                unset($this->object_new['subnet'], $this->object_new['mask']);
            }
            if (@$this->object_old['isFolder'] == "1") {
                unset($this->object_old['subnet'], $this->object_old['mask']);
            }
        } # remove order fields
    ...
    private function changelog_calculate_edit_diff()
    {
        ...
        foreach ($this->object_new as $k => $v) {
            ...
            if ($this->object_old[$k] != $v && ($this->object_old[$k] != str_replace("\'", "'", $v))) {
                //empty
                if (strlen(@$this->object_old[$k]) == 0) {
                    $this->object_old[$k] = "NULL";
                }
                if (strlen(@$v) == 0) {
                    $v = "NULL";
                }
                ...
                //tag change
                if ($k == 'state') {
                    $v = $this->changelog_format_tag_diff($k, $v);
                } //section change
                elseif ($k == 'sectionIdNew') {
                    $v = $this->changelog_format_section_diff($k, $v);
                } //section change
                elseif ($k == 'sectionId') {
                    $v = $this->changelog_format_section_diff($k, $v);
                } //subnet change
                elseif ($k == "subnet") {
                    $v = $this->changelog_format_master_subnet_diff($k, $v);
                } //master subnet change
                elseif ($k == "masterSubnetId") {
                    $v = $this->changelog_format_master_subnet_diff($k, $v);
                } //device change
                elseif ($k == 'switch') {
                    $v = $this->changelog_format_device_diff($k, $v);
                } //device change
                elseif ($k == 'device') {
                    $v = $this->changelog_format_device_diff($k, $v);
                } //vlan
                elseif ($k == 'vlanId') {
                    $v = $this->changelog_format_vlan_diff($k, $v);
                } //vrf
                elseif ($k == 'vrfId') {
                    $v = $this->changelog_format_vrf_diff($k, $v);
                } //location
                elseif ($k == 'location_item') {
                    $v = $this->changelog_format_location_diff($k, $v);
                } //location
                elseif ($k == 'location') {
                    $v = $this->changelog_format_location_diff($k, $v);
                } //master section change
                elseif ($k == 'masterSection') {
                    $v = $this->changelog_format_master_section_diff($k, $v);
                } //permission change
                elseif ($k == "permissions") {
                    $v = $this->changelog_format_permission_diff($k, $v);
                } // nameserver index
                elseif ($k == "nameserverId") {
                    $v = $this->changelog_format_ns_diff($k, $v);
                }
                // make booleans
    ...
    private function changelog_make_booleans($k, $v)
    {
        ...
        $keys['section'] = ["strictMode", "showVLAN", "showVRF"];
        $keys['subnet'] = ["allowRequests", "showName", "pingSubnet", "discoverSubnet", "DNSrecursive", "DNSrecords", "isFull"];
    ...
    private function changelog_reformat_add_diff()
    {
        foreach ($this->object_new as $k => $v) {
            //tag change
            if ($k == 'state') {
                $this->object_new[$k] = $this->changelog_format_tag_diff($k, $v);
            } //section change
            elseif ($k == 'sectionId') {
                $this->object_new[$k] = $this->changelog_format_section_diff($k, $v);
            } //section change
            elseif ($k == 'section') {
                $this->object_new[$k] = $this->changelog_format_section_diff($k, $v);
            } //master subnet change
            elseif ($k == "subnetId") {
                $this->object_new[$k] = $this->changelog_format_master_subnet_diff($k, $v);
            } //master subnet change
            elseif ($k == "masterSubnetId") {
                $this->object_new[$k] = $this->changelog_format_master_subnet_diff($k, $v);
            } //device change
            elseif ($k == 'switch') {
                $this->object_new[$k] = $this->changelog_format_device_diff($k, $v);
            } //device change
            elseif ($k == 'device') {
                $this->object_new[$k] = $this->changelog_format_device_diff($k, $v);
            } //vlan
            elseif ($k == 'vlanId') {
                $this->object_new[$k] = $this->changelog_format_vlan_diff($k, $v);
            } //vrf
            elseif ($k == 'vrfId') {
                $this->object_new[$k] = $this->changelog_format_vrf_diff($k, $v);
            } //location
            elseif ($k == 'location_item') {
                $this->object_new[$k] = $this->changelog_format_location_diff($k, $v);
            } //location
            elseif ($k == 'location') {
                $this->object_new[$k] = $this->changelog_format_location_diff($k, $v);
            } //master section change
            elseif ($k == 'masterSection') {
                $this->object_new[$k] = $this->changelog_format_master_section_diff($k, $v);
            } //permission change
            elseif ($k == "permissions") {
                $this->object_new[$k] = $this->changelog_format_permission_diff($k, $v);
            } // nameserver index
            elseif ($k == "nameserverId") {
                $this->object_new[$k] = $this->changelog_format_ns_diff($k, $v);
            }
        }
    }
    ...
    private function changelog_reformat_delete_diff()
    {
        foreach ($this->object_old as $k => $v) {
            //tag change
            if ($k == 'state') {
                $this->object_old[$k] = $this->changelog_format_tag_diff($k, $v);
            } //section change
            elseif ($k == 'section') {
                $this->object_old[$k] = $this->changelog_format_section_diff($k, $v);
            } //section change
            elseif ($k == 'sectionId') {
                $this->object_old[$k] = $this->changelog_format_section_diff($k, $v);
            } //subnet change
            elseif ($k == "subnetId") {
                $this->object_old[$k] = $this->changelog_format_master_subnet_diff($k, $v);
            } //master subnet change
            elseif ($k == "masterSubnetId") {
                $this->object_old[$k] = $this->changelog_format_master_subnet_diff($k, $v);
            } //device change
            elseif ($k == 'switch') {
                $this->object_old[$k] = $this->changelog_format_device_diff($k, $v);
            } //device change
            elseif ($k == 'device') {
                $this->object_old[$k] = $this->changelog_format_device_diff($k, $v);
            } //vlan
            elseif ($k == 'vlanId') {
                $this->object_old[$k] = $this->changelog_format_vlan_diff($k, $v);
            } //vrf
            elseif ($k == 'vrfId') {
                $this->object_old[$k] = $this->changelog_format_vrf_diff($k, $v);
            } //location
            elseif ($k == 'location_item') {
                $this->object_old[$k] = $this->changelog_format_location_diff($k, $v);
            } //location
            elseif ($k == 'location') {
                $this->object_old[$k] = $this->changelog_format_location_diff($k, $v);
            } //master section change
            elseif ($k == 'masterSection') {
                $this->object_old[$k] = $this->changelog_format_master_section_diff($k, $v);
            } //permission change
            elseif ($k == "permissions") {
                $this->object_old[$k] = $this->changelog_format_permission_diff($k, $v);
            } // nameserver index
            elseif ($k == "nameserverId") {
                $this->object_old[$k] = $this->changelog_format_ns_diff($k, $v);
            }
        }
    }
    ...
    private function syslog_write_changelog($changelog)
    {
        ...
        if ($this->object_action == "add") {
            $obj_id = $this->object_new['id'];
        } else {
            $obj_id = $this->object_old['id'];
        }
    ...
    private function get_active_user_id()
    {
        ...
        if ($this->user_id === null) {
            ...
            if (!isset($_SESSION['ipamusername'])) {
                ...
                try {
                    $user_id = $this->Database->getObjectQuery("select * from `users` where `username` = ? limit 1", [$_SESSION['ipamusername']]);
                } catch (Exception $e) {
    ...
    private function changelog_write_to_db($changelog)
    {
        ...
        if ($this->user === null && php_sapi_name() == "cli") {
            $this->user_id = 1;
        }
        ...
        # execute
        try {
            $this->Database->insertObject("changelog", $values);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
    ...
    public function changelog_send_mail($changelog)
    {
        ...
        if ($this->object_type == "section") {
            ...
            $details = "<a style='font-family:Helvetica, Verdana, Arial, sans-serif; font-size:12px;color:#a0ce4e;' href='" . $this->createURL() . create_link("subnets", $this->object_orig['sectionId'], $obj_details['id']) . "'>" . $this->Subnets->transform_address($obj_details['subnet'], "dotted") . "/" . $obj_details['mask'] . " (" . $obj_details['description'] . ") - id " . $obj_details['id'] . "</a>";
        } elseif ($this->object_type == "folder") {
            $details = "<a style='font-family:Helvetica, Verdana, Arial, sans-serif; font-size:12px;color:#a0ce4e;' href='" . $this->createURL() . create_link("folder", $this->object_orig['sectionId'], $obj_details['id']) . "'>" . $obj_details['description'] . " - id " . $obj_details['id'] . "</a>";
    ...
    public function fetch_all_changelogs($filter = false, $expr, $limit = 100)
    {
        ...
        # fetch
        try {
            $logs = $this->Database->getObjectsQuery($query, ["expr" => $expr]);
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), false);
            return false;
        }
    ...
    public function fetch_changelog($id)
    {
        # limit check
        if (!is_numeric($id)) {
            $this->Result->show("danger", "Invalid ID", true);
            return false;
        }
        ...
        # fetch
        try {
            $logs = $this->Database->getObjectQuery($query, ["id" => $id]);
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), false);
            return false;
        }
    ...
    public function fetch_changlog_entries($object_type, $coid, $long = false, $limit = 50)
    {
        ...
        switch ($object_type) {
            ...
            // error
            default:
                $this->Result->show("danger", "Invalid object type", true);
                return false;
        }
        ...
        # fetch
        try {
            $logs = $this->Database->getObjectsQuery($query, [$coid, $object_type]);
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), false);
            return false;
        }
