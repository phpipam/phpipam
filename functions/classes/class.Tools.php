...
class Tools extends Common_functions
{
    ...
    public function fetch_vlans_and_subnets($domainId = 1)
    {
        ...
        # fetch
        try {
            $vlans = $this->Database->getObjectsQuery($query, [$domainId]);
        } catch (Exception $e) {
    ...
    public function search_addresses($search_term, $high = "", $low = "", $custom_fields = [])
    {
        ...
        # fetch
        try {
            $result = $this->Database->getObjectsQuery($query, ["low" => $low, "high" => $high, "search_term" => "%$search_term%", "tags" => $tags]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    private function search_subnets_range($search_term, $high, $low, $custom_fields = [])
    {
        ...
        # fetch
        try {
            $result = $this->Database->getObjectsQuery($query, ["low" => $low, "high" => $high, "search_term" => "%$search_term%"]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    private function search_subnets_inside($high, $low)
    {
        if ($low == $high) {
            ...
            # filter
            $ids = sizeof(@$ids) > 0 ? array_filter($ids) : [];
            ...
            # return
            return sizeof(@$result) > 0 ? array_filter($result) : [];
        } else {
            return [];
        }
    }
    ...
    public function search_vlans($search_term, $custom_fields = [])
    {
        ...
        # fetch
        try {
            $search = $this->Database->getObjectsQuery($query, ["search_term" => "%$search_term%"]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function fetch_full_field_definition($table, $field)
    {
        ...
        # fetch
        try {
            $field_data = $this->Database->getObjectQuery("show full columns from `$table` where `Field` = ?;", [$field]);
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), false);
            return false;
        }
    ...
    {
        try {
            $subnets = $this->Database->getObjectsQuery("SELECT * FROM `subnets` where `allowRequests`=1 and COALESCE(`isFull`,0) != 1 ORDER BY `subnet`;");
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), false);
            return false;
        }
    ...
    public function table_exists($tablename, $quit = false)
    {
        ...
        try {
            $count = $this->Database->getObjectQuery($query, [$tablename]);
        } catch (Exception $e) {
    ...
    {
        try {
            return $indexes = $this->Database->getObjectsQuery("SHOW INDEX from `$table` where `Key_name` != 'PRIMARY';");
        } catch (Exception $e) {
    ...
    public function verify_linked_field_indexes($linked_field)
    {
        ...
        foreach ($table_indexes as $i) {
            ...
            try {
                $this->Database->runQuery("ALTER TABLE `ipaddresses` DROP INDEX $i->Key_name;");
            } catch (Exception $e) {
                $this->Result->show("danger", $e->getMessage(), true);
            }
        ...
        try {
            $this->Database->runQuery("ALTER TABLE `ipaddresses` ADD INDEX ($linked_field);");
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), true);
        }
    ...
    public function check_number_duplicates($prefix = false, $number = false)
    {
        if ($prefix === false && $number === false) {
            ...
            # fetch
            try {
                $cnt = $this->Database->getObjectQuery($query, [$prefix, $number]);
            } catch (Exception $e) {
                $this->Result->show("danger", _("Error: ") . $e->getMessage());
                return false;
            }
    ...
    public function fetch_all_prefixes($master = false, $recursive = false)
    {
        ...
        # fetch
        try {
            $prefixes = $this->Database->getObjectsQuery($query, $params);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function fetch_all_circuits($custom_circuit_fields = [])
    {
        ...
        // fetch
        try {
            $circuits = $this->Database->getObjectsQuery(implode("\n", $query), []);
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), true);
        }
        // return
    ...
    public function fetch_all_device_circuits($device_id)
    {
        ...
        // fetch
        try {
            $circuits = $this->Database->getObjectsQuery($query, ["deviceid" => $device_id]);
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), true);
        }
        // return
    ...
    public function fetch_routing_subnets($type = "bgp", $id = 0, $cnt = false)
    {
        ...
        // fetch
        try {
            $subnets = $this->Database->getObjectsQuery($query, [$type, $id]);
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), true);
        }
        // return
    ...
    public function parse_import_file($filetype = "xls", $subnet = object, $custom_address_fields)
    {
        ...
        if (strtolower($filetype) == "csv") {
            $outFile = $this->parse_import_file_csv();
        } # XLS
     ...
     * @return void
     */
    private function parse_validate_file($outFile = [], $subnet = object)
    {
        $result = [];
        # present ?
        if (sizeof($outFile) > 0) {
            foreach ($outFile as $k => $line) {
                ...
                if ($subnet->isFolder != "1") {
                    ...
                    $ipsm = "32";
                    if (!filter_var($field[0], FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) === false) {
                        $ipsm = "128";
                    }
                    if ($this->Subnets->is_subnet_inside_subnet($field[0] . "/" . $ipsm, $this->transform_address($subnet->subnet, "dotted") . "/" . $subnet->mask) == false) {
                        $class = "danger";
                        $errors++;
                    }
                }
    ...
    public function count_subnets($type = "IPv4")
    {
        # set proper query
        if ($type == "IPv4") {
            $query = 'select count(*) as count from `ipaddresses` where cast(`ip_addr` as UNSIGNED) <= 4294967295;';
        } elseif ($type == "IPv6") {
            $query = 'select count(*) as count from `ipaddresses` where cast(`ip_addr` as UNSIGNED) >  4294967295;';
        }
    ...
    public function fetch_top_subnets($type, $limit = "10", $perc = false)
    {
        ...
        try {
            ...
            !$this->debugging ?: $this->Result->show("danger", $e->getMessage(), true);
            return false;
        }
    ...
    public function fetch_addresses_for_export()
    {
        # fetch
        try {
            $addresses = $this->Database->getObjectsQuery("select `id`,`subnetId`,`ip_addr`,`hostname` from `ipaddresses` where length(`hostname`)>1 order by `subnetId` asc;");
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), false);
            return false;
        }
