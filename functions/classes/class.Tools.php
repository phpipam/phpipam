...
class Tools extends Common_functions {
	 ...
	 * @return array
	 */
	public function search_locations ($search_term, $custom_fields = array()) {
		# query
		$query[] = "select * from `locations` where `name` like :search_term or `description` like :search_term ";
		# custom
	    if(sizeof($custom_fields) > 0) {
			foreach($custom_fields as $myField) {
				$myField['name'] = $this->Database->escape($myField['name']);
		...
		# fetch
		try { $search = $this->Database->getObjectsQuery('locations', $query, array("search_term"=>"%$search_term%")); }
		catch (Exception $e) {
			$this->Result->show("danger", _("Error: ").$e->getMessage());
			return false;
		}
