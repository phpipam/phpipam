...
class Admin extends Common_functions
{
        ...
        if (php_sapi_name() != "cli") {
            ...
            if ($this->User->is_admin(false) !== true && $this->admin_required === true) {
                ...
                if (@$_SERVER['HTTP_X_REQUESTED_WITH'] == "XMLHttpRequest") {
                    $this->Result->show("danger", _("Administrative privileges required"), true, true);
                } else {
                    $this->Result->show("danger", _("Administrative privileges required"), true);
                }
            }
        }
    }
    ...
    {
        try {
            $this->Database->runQuery("update `$table` set `$field` = ? where `$field` = ?;", [$new_value, $old_value]);
        } catch (Exception $e) {
    ...
    public function truncate_table($table = null)
    {
        ...
        if (is_null($table) || strlen($table) == 0) return false;
            ...
            try {
                $this->Database->emptyTable($table);
            } catch (Exception $e) {
    ...
    public function add_group_to_user($gid, $uid)
    {
        ...
        # update
        if (!$this->update_user_groups($uid, $g)) {
            return false;
        } else {
            return true;
        }
    ...
    public function object_modify($table, $action = null, $field = "id", $values = [], $values_log = [])
    {
        ...
        if (sizeof($values_log) == 0) {
            $values_log = $values;
        }
        
        # execute based on action
        if ($action == "add") {
            return $this->object_add($table, $values, $values_log);
        } elseif ($action == "edit") {
            return $this->object_edit($table, $field, $values, $values_log);
        } elseif ($action == "edit-multiple") {
            return $this->object_edit_multiple($table, $field, $values);
        }        //$field = array of ids
        elseif ($action == "delete") {
            return $this->object_delete($table, $field, $values[$field]);
        } else {
    ...
    private function object_add($table, $values, $values_log)
    {
        ...
        # execute
        try {
            $this->Database->insertObject($table, $values);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
    ...
    private function object_edit($table, $key = "id", $values, $values_log = [])
    {
        ...
        # execute
        try {
            $this->Database->updateObject($table, $values, $key);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
    ...
    private function object_edit_multiple($table, $ids, $values)
    {
        ...
        # execute
        try {
            $this->Database->updateMultipleObjects($table, $ids, $values);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
    ...
    private function object_delete($table, $field = "id", $id)
    {
        # execute
        try {
            $this->Database->deleteRow($table, $field, $id);
        } catch (Exception $e) {
    ...
    public function replace_fields($field, $search, $replace)
    {
        ...
        if ($count > 0) {
            # update
            try {
                $this->Database->runQuery("update `ipaddresses` set `$field` = replace(`$field`, ?, ?);", [$search, $replace]);
            } catch (Exception $e) {
    ...
    public function update_custom_field_definition($field)
    {
        ...
        if ($field['fieldType'] == "varchar" || $field['fieldType'] == "text" || $field['fieldType'] == "set" || $field['fieldType'] == "enum") {
        ...
        if ($field['action'] == "delete") {
            $query = "ALTER TABLE `$field[table]` DROP `$field[oldname]`;";
        } elseif ($field['action'] == "edit" && @$field['NULL'] == "NO") {
            $query = "ALTER TABLE `$field[table]` CHANGE COLUMN `$field[oldname]` `$field[name]` $field[ftype] $charset DEFAULT :default NOT NULL COMMENT :comment;";
        } elseif ($field['action'] == "edit") {
            $query = "ALTER TABLE `$field[table]` CHANGE COLUMN `$field[oldname]` `$field[name]` $field[ftype] $charset DEFAULT :default COMMENT :comment;";
        } elseif ($field['action'] == "add" && @$field['NULL'] == "NO") {
            $query = "ALTER TABLE `$field[table]` ADD COLUMN 	`$field[name]` 					$field[ftype] $charset DEFAULT :default NOT NULL COMMENT :comment;";
        } elseif ($field['action'] == "add") {
            $query = "ALTER TABLE `$field[table]` ADD COLUMN 	`$field[name]` 					$field[ftype] $charset DEFAULT :default NULL COMMENT :comment;";
        } else {
            return false;
        }
        ...
        # execute
        try {
            $res = $this->Database->runQuery($query, $params);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
    ...
    public function save_custom_fields_filter($table, $filtered_fields)
    {
        ...
        $hidden_array = strlen($this->settings->hiddenCustomFields) > 0 ? json_decode($this->settings->hiddenCustomFields, true) : [];
        
        # set new array for table
        if (is_null($filtered_fields)) {
            unset($hidden_array[$table]);
        } else {
            $hidden_array[$table] = $filtered_fields;
        }
        ...
        # update database
        try {
            $this->object_edit("settings", $key = "id", ["id" => 1, "hiddenCustomFields" => $hidden_json]);
        } catch (Exception $e) {
