...
class SubnetsTable {
	...
	private function table_row($subnet) {
		...
		if ($this->Tools->settings->enableCustomers == 1) {
			...
			$tr['customer'] = $customer===false ? "/" : $customer->title." <a target='_blank' href='".create_link("tools","customers",unescape_input($customer->title))."'><i class='fa fa-external-link'></i></a>";
		}
