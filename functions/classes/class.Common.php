...
class Common_functions
{
    ...
    {
        try {
            $cnt = $this->Database->getGroupBy($table, $field);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function fetch_multiple_objects($table, $field, $value, $sortField = 'id', $sortAsc = true, $like = false, $result_fields = "*")
    {
        ...
        if (is_null($table) || strlen($table) == 0) return false;
            ...
            try {
                $res = $this->Database->findObjects($table, $field, $value, $sortField, $sortAsc, $like, false, $result_fields);
            } catch (Exception $e) {
                $this->Result->show("danger", _("Error: ") . $e->getMessage());
                return false;
            }
    ...
    public function fetch_object($table = null, $method = null, $value)
    {
        ...
        try {
            $res = $this->Database->getObjectQuery("SELECT * from `$table` where `$method` = ? limit 1;", [$value]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function update_latlng($id, $lat, $lng)
    {
        # execute
        try {
            $this->Database->updateObject("locations", ["id" => $id, "lat" => $lat, "long" => $lng], "id");
        } catch (Exception $e) {
            return false;
        }
        return true;
    ...
    public function create_custom_field_input_date($field, $object, $timepicker_index, $disabled_text)
    {
        ...
        if (!isset($object->{$field['name']})) {
            $html[] = ' <input type="text" class="' . $class . ' form-control input-sm input-w-auto" data-format="' . $format . '" name="' . $field['nameNew'] . '" maxlength="' . $size . '" rel="tooltip" data-placement="right" title="' . $field['Comment'] . '" ' . $disabled_text . '>' . "\n";
        } else {
            $html[] = ' <input type="text" class="' . $class . ' form-control input-sm input-w-auto" data-format="' . $format . '" name="' . $field['nameNew'] . '" maxlength="' . $size . '" value="' . $this->strip_xss($object->{$field['name']}) . '" rel="tooltip" data-placement="right" title="' . $field['Comment'] . '" ' . $disabled_text . '>' . "\n";
        }
    ...
    public function print_breadcrumbs($Section, $Subnet, $req, $Address = null)
    {
        # subnets
        if ($req['page'] == "subnets") {
            $this->print_subnet_breadcrumbs($Subnet, $req, $Address);
        } # folders
