...
class FirewallZones extends Common_functions
{
    ...
    private function generate_numeric_zone_name($zoneLength, $zoneGenerator)
    {
        ...
        # execute
        try {
            $maxZone = $this->Database->getObjectsQuery('SELECT MAX(CAST(zone as UNSIGNED)) as zone FROM firewallZones WHERE generator NOT LIKE 2;');
        } catch (Exception $e) {
    ...
    private function validate_text_zone_name($values)
    {
        ...
        # execute
        try {
            $uniqueZone = $this->Database->getObjectsQuery($query, $params);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
    ...
    public function get_zone_mappings()
    {
        ...
        try {
            $mappings = $this->Database->getObjectsQuery('SELECT
        ...
        try {
            $networkInformation = $this->Database->getObjectsQuery('SELECT
    ...
    public function check_zone_mapping($zoneId)
    {
        ...
        try {
            $mapping = $this->Database->getObjectsQuery('SELECT id FROM firewallZoneMapping WHERE zoneId = ?;', $zoneId);
        } # throw exception
        catch (Exception $e) {
            $this->Result->show("danger", _("Database error: ") . $e->getMessage());
        }
    ...
    public function get_zones()
    {
        ...
        try {
            $zones = $this->Database->getObjectsQuery('SELECT * FROM firewallZones;');
        } # throw exception
        catch (Exception $e) {
            $this->Result->show("danger", _("Database error: ") . $e->getMessage());
        }
    ...
    public function get_zone($id)
    {
        ...
        try {
            $zone = $this->Database->getObjectsQuery('SELECT * FROM firewallZones WHERE id = ?;', $id);
        } # throw exception
        catch (Exception $e) {
            $this->Result->show("danger", _("Database error: ") . $e->getMessage());
        }
        ...
        foreach ($networkInformation as $network) {
            ...
            if ($network->subnetIsFolder == 1) {
                ...
                if ($network->subnetDescription) {
                    print $this->Subnets->transform_to_dotted($network->subnet) . '/' . $network->subnetMask . ' (' . $network->subnetDescription . ')</td>';
                } else {
                    print $this->Subnets->transform_to_dotted($network->subnet) . '/' . $network->subnetMask . '</td>';
                }
            }
            print '</tr>';
            $i++;
    ...
    public function check_zone_network($subnetId)
    {
        ...
        try {
            $networkInformation = $this->Database->getObjectsQuery('SELECT * FROM firewallZoneSubnet WHERE subnetId = ?;', $subnetId);
        } # throw exception
        catch (Exception $e) {
            $this->Result->show("danger", _("Database error: ") . $e->getMessage());
        }
    ...
    public function add_zone_network($zoneId, $subnetId)
    {
        ...
        if (!sizeof($networkInformation) > 0) {
            ...
            # try to fetch all subnet and vlan informations for this zone
            try {
                $addRow = $this->Database->insertObject("firewallZoneSubnet", $params);
            } # throw exception
            catch (Exception $e) {
                $this->Result->show("danger", _("Database error: ") . $e->getMessage());
            }
    ...
    public function delete_zone_network($zoneId, $subnetId)
    {
        # try to fetch all subnet and vlan informations for this zone
        try {
            $deleteRow = $this->Database->deleteRow("firewallZoneSubnet", "zoneId", $zoneId, "subnetId", $subnetId);
        } # throw exception
        catch (Exception $e) {
            $this->Result->show("danger", _("Database error: ") . $e->getMessage());
        }
    ...
    public function modify_zone($action, $values)
    {
        ...
        # execute based on action
        if ($action == "add") {
            return $this->zone_add($values, $network);
        } elseif ($action == "edit") {
            return $this->zone_edit($values);
        } elseif ($action == "delete") {
            return $this->zone_delete($values['id']);
        } else {
    ...
    private function zone_add($values, $network)
    {
        ...
        # execute insert
        try {
            $this->Database->insertObject("firewallZones", $values);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
        ...
        try {
            $lastId = $this->Database->getObjectsQuery("SELECT MAX(id) AS id FROM firewallZones WHERE zone = ? ;", $values['zone']);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
        ...
        if ($network) {
            foreach ($network as $subnetId) {
                ...
                try {
                    $this->Database->insertObject("firewallZoneSubnet", $values);
                } catch (Exception $e) {
                    $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
                    return false;
                }
    ...
    private function zone_edit($values)
    {
        # execute
        try {
            $this->Database->updateObject("firewallZones", $values, "id");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
    ...
    private function zone_delete($id)
    {
        ...
        # delete mappings
        try {
            $this->Database->deleteRow("firewallZoneMapping", "zoneId", $id);
        } catch (Exception $e) {
        ...
        # delete zone
        try {
            $this->Database->deleteRow("firewallZones", "id", $id);
        } catch (Exception $e) {
    ...
    public function modify_mapping($action, $values)
    {
        ...
        # execute based on action
        if ($action == "add") {
            return $this->mapping_add($values);
        } elseif ($action == "edit") {
            return $this->mapping_edit($values);
        } elseif ($action == "delete") {
            return $this->mapping_delete($values['id']);
        } else {
            return $this->Result->show("danger", _("Invalid action"), true);
        }
    }
    ...
    private function mapping_add($values)
    {
        ...
        # execute
        try {
            $this->Database->insertObject("firewallZoneMapping", $values);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
    ...
    private function mapping_edit($values)
    {
        # execute
        try {
            $this->Database->updateObject("firewallZoneMapping", $values, "id");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
    ...
    private function mapping_delete($id)
    {
        ...
        # delete mapping
        try {
            $this->Database->deleteRow("firewallZoneMapping", "id", $id);
        } catch (Exception $e) {
    ...
    public function get_zone_mapping($id)
    {
        # try to fetch id specific zone mapping
        try {
            $mapping = $this->Database->getObjectsQuery('SELECT
						firewallZones.id AS id,
						firewallZones.generator AS generator,
						firewallZones.length AS length,
						firewallZones.padding AS padding,
        ...
        foreach ($firewallZoneSettings['pattern'] as $pattern) {
            switch ($pattern) {
                case 'patternIndicator':
                    if ($zone->indicator == 0) {
                        $firewallAddressObject = $firewallAddressObject . $firewallZoneSettings['indicator'][0];
                    } else {
                        $firewallAddressObject = $firewallAddressObject . $firewallZoneSettings['indicator'][1];
                    }
                    break;
    ...
    public function get_zone_subnet_info($id)
    {
        ...
        try {
            $info = $this->Database->getObjectsQuery('SELECT
						firewallZones.zone as zone,
						firewallZones.padding as padding,
						firewallZones.length as length,
						firewallZones.indicator as indicator,
    ...
    public function update_address_object($subnetId, $IPId, $dnsName)
    {
        ...
        if ($address_old->firewallAddressObject != $firewallAddressObject) {
            ...
            try {
                $this->Database->updateObject("ipaddresses", $values, "id", "subnetId");
            } catch (Exception $e) {
                $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
                return false;
            }
    ...
    public function update_address_objects($subnetId)
    {
        ...
        try {
            $ipaddresses = $this->Database->getObjectsQuery('SELECT id, hostname FROM ipaddresses WHERE subnetId = ? ', $subnetId);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
