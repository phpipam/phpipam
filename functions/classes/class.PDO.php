...
/**
     ...
     * @access protected
     */
    protected $charset = 'utf8';        // needed for DB check
    /**
     * pdo
     * (default value: null)
     *
    ...
    public function getObjects($tableName, $sortField = 'id', $sortAsc = true, $numRecords = null, $offset = 0, $class = 'stdClass')
    {
        ...
        if ($tableName == 'vlans' && $sortField == 'id') {
            $sortField = "vlanId";
        }
        if ($tableName == 'vrf' && $sortField == 'id') {
            $sortField = "vrfId";
        }
     ...
     * @return void
     */
    public function getObjectsQueryIncremental($query = null, $values = [], $callback = null)
    {
        if (!$this->isConnected()) $this->connect();
     ...
     * @return void
     */
    public function getValueQuery($query = null, $values = [], $class = 'stdClass')
    {
     ...
     * @return void
     */
    public function getObjectQuery($query = null, $values = [], $class = 'stdClass')
    {
        if (!$this->isConnected()) $this->connect();
    ...
    public function findObjects($table, $field, $value, $sortField = 'id', $sortAsc = true, $like = false, $negate = false, $result_fields = "*")
    {
        ...
        // change sort fields for vlans and vrfs. ugly :/
        if ($table == 'vlans' && $sortField == 'id') {
            $sortField = "vlanId";
        }
        if ($table == 'vrf' && $sortField == 'id') {
            $sortField = "vrfId";
        }
        
        // subnets
        if ($table == 'subnets' && $sortField == 'subnet') {
            return $this->getObjectsQuery('SELECT ' . $result_fields . ' FROM `' . $table . '` WHERE `' . $field . '`' . $negate_operator . $operator . '? ORDER BY LPAD(`subnet`,39,0) ' . ($sortAsc ? '' : 'DESC') . ';', [$value]);
        } else {
            return $this->getObjectsQuery('SELECT ' . $result_fields . ' FROM `' . $table . '` WHERE `' . $field . '`' . $negate_operator . $operator . '? ORDER BY `' . $sortField . '` ' . ($sortAsc ? '' : 'DESC') . ';', [$value]);
        }
    }
     ...
     * @return void
     */
    public function getObjectsQuery($query = null, $values = [], $class = 'stdClass')
    {
        if (!$this->isConnected()) $this->connect();
    ...
    public function findObject($table, $field, $value)
    {
        ...
        return $this->getObjectQuery('SELECT * FROM `' . $table . '` WHERE `' . $field . '` = ? LIMIT 1;', [$value]);
    }
     ...
     * @return void
     */
    public function getList($query = null, $values = [], $class = 'stdClass')
    {
...
class Database_PDO extends DB
{
    ...
    /**
     * SSL options for db connectionnig!
     * (default value: false)
     *
     * @var bool
     * @access public
    ...
    /**
     * flag if installation isdefault value: false)
     *
     * @var bool
     * @access protected
     */
    ...
    public function getFieldInfo($tableName = false, $field = false)
    {
        ...
        $field = $this->escape($field);
        // fetch and return
        return $this->getObjectQuery("SHOW FIELDS FROM `$tableName` where Field = ?", [$field]);
