...
class Install extends Common_functions
{
    ...
    private function drop_database()
    {
        ...
        # execute
        try {
            $this->Database_root->runQuery($query);
        } catch (Exception $e) {
        ...
        foreach ($queries as $q) {
            ...
            if (strlen($q) > 0) {
                try {
                    ...
                    //drop database
                    try {
                        $this->Database_root->runQuery("drop database if exists `" . $this->db['name'] . "`;");
                    } catch (Exception $e) {
    ...
    public function check_table($table, $redirect = false)
    {
        ...
        # try to fetch count
        try {
            $table = $this->Database->getObjectQuery($query);
        } catch (Exception $e) {
            if ($redirect === true) $this->redirect_to_install(); else return false;
        }
    ...
    {
        try {
            $this->Database->updateObject("users", ["password" => $adminpass, "passChange" => "No", "username" => "Admin"], "username");
        } catch (Exception $e) {
    ...
    {
        try {
            $this->Database->updateObject("settings", ["siteTitle" => $siteTitle, "siteURL" => $siteURL, "id" => 1], "id");
        } catch (Exception $e) {
            $this->Result->show("danger", $e->getMessage(), false);
        }
        return true;
...
class Upgrade extends Install
{
    ...
    {
        if ($this->old_version == VERSION . DBVERSION) {
            $this->Result->show("danger", "Database already at latest version", true);
        } else {
            # check db connection
            if ($this->check_db_connection(false) === false) {
                $this->Result->show("danger", "Cannot connect to database", true);
            } # execute
            else {
