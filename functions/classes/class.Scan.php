...
class Scan extends Common_functions
{
    ...
    public function reset_scan_method($method)
    {
        ...
        if (!in_array($method, $possible)) {
            //die or print?
            if ($this->icmp_exit) {
                die(json_encode(["status" => 1, "error" => "Invalid scan method"]));
            } else {
                $this->Result->show("danger", "Invalid scan method", true);
            }
        } //ok
        else {
    ...
    {
        if (PHP_OS == "FreeBSD" || PHP_OS == "NetBSD") {
            $this->os_type = "FreeBSD";
        } elseif (PHP_OS == "Linux" || PHP_OS == "OpenBSD") {
            $this->os_type = "Linux";
        } elseif (PHP_OS == "WIN32" || PHP_OS == "Windows" || PHP_OS == "WINNT") {
    ...
    public function ping_address_method_fping($address)
    {
        ...
        if ($this->identify_address($address) == "IPv6") {
            $this->settings->scanFPingPath = $this->settings->scanFPingPath . "6";
        }
    ...
    public function ping_update_lastseen($id, $datetime = null)
    {
        ...
        # execute
        try {
            $this->Database->updateObject("ipaddresses", ["id" => $id, "lastSeen" => $datetime], "id");
        } catch (Exception $e) {
    ...
    public function ping_update_scanagent_checktime($id, $date = false)
    {
        ...
        # execute
        try {
            $this->Database->updateObject("scanAgents", ["id" => $id, "last_access" => date("Y-m-d H:i:s")], "id");
        } catch (Exception $e) {
        }
    }
    ...
    public function update_subnet_scantime($subnet_id, $datetime = false)
    {
        ...
        // update
        try {
            $this->Database->updateObject("subnets", ["id" => $subnet_id, "lastScan" => $datetime], "id");
        } catch (Exception $e) {
        }
    }
    ...
    public function update_subnet_discoverytime($subnet_id, $datetime = false)
    {
        ...
        // update
        try {
            $this->Database->updateObject("subnets", ["id" => $subnet_id, "lastDiscovery" => $datetime], "id");
        } catch (Exception $e) {
        }
    }
        ...
        if (is_numeric($address_id)) {
            ...
            if ($last_seen_date !== false && !is_null($last_seen_date) && strlen($last_seen_date) > 2 && $last_seen_date != "0000-00-00 00:00:00" && $last_seen_date != "1970-01-01 00:00:01" && $last_seen_date != "1970-01-01 01:00:00") {
                ...
                if (!($tag_id == 1 && $old_tag_id == 3)) {
                    try {
                        $this->Database->updateObject("ipaddresses", ["id" => $address_id, "state" => $tag_id], "id");
                    } catch (Exception $e) {
                        return false;
                    }
                } else {
    ...
    public function prepare_addresses_to_scan($type, $subnet, $die = true)
    {
        ...
        if ($type == "discovery") {
            return is_numeric($subnet) ? $this->prepare_addresses_to_discover_subnetId($subnet, $die) : $this->prepare_addresses_to_discover_subnet($subnet, $die);
        } # update addresses statuses
        elseif ($type == "update") {
            return $this->prepare_addresses_to_update($subnet);
        } # fail
        else {
            die(json_encode(["status" => 1, "error" => "Invalid scan type provided"]));
        }
    }
    ...
    public function prepare_addresses_to_discover_subnetId($subnetId, $die)
    {
        ...
        if ($subnet === false) {
            if ($die) {
                die(json_encode(["status" => 1, "error" => "Invalid subnet ID provided"]));
            } else {
                return [];
            }
        }
        ...
        if ($Subnets->get_max_hosts($subnet->mask, "IPv4") > 4094 && php_sapi_name() != "cli")
            if ($die) {
                die(json_encode(["status" => 1, "error" => "Scanning from GUI is only available for subnets up to /20 or 4094 hosts!"]));
            } else {
                return [];
            }
        ...
        if (sizeof($ip) == 0) {
            if ($die) {
                die(json_encode(["status" => 1, "error" => "Didn't find any address to scan!"]));
            } else {
                return [];
            }
        }
    ...
    protected function ping_address_method_ping($address)
    {
        ...
        # if ipv6 append 6
        if ($this->identify_address($address) == "IPv6") {
            $this->settings->scanPingPath = $this->settings->scanPingPath . "6";
        }
        
        # set ping command based on OS type
        if ($this->os_type == "FreeBSD") {
            $cmd = $this->settings->scanPingPath . " -c $this->icmp_count -W " . ($this->icmp_timeout * 1000) . " $address 1>/dev/null 2>&1";
        } elseif ($this->os_type == "Linux") {
            $cmd = $this->settings->scanPingPath . " -c $this->icmp_count -W $this->icmp_timeout $address 1>/dev/null 2>&1";
        } elseif ($this->os_type == "Windows") {
            $cmd = $this->settings->scanPingPath . " -n $this->icmp_count -w " . ($this->icmp_timeout * 1000) . " $address";
        } else {
            $cmd = $this->settings->scanPingPath . " -c $this->icmp_count -n $address 1>/dev/null 2>&1";
        }
    ...
    private function ping_verify_path($path)
    {
        ...
        if ($this->os_type == "Windows") {
            if (!file_exists('"' . $path . '"')) {
                if ($this->icmp_exit) {
                    exit  ($this->ping_exit_explain(1000));
                } else {
                    return $this->Result->show("danger", _($this->ping_exit_explain(1000)), true);
                }
            }
        } else {
            if (!file_exists($path)) {
