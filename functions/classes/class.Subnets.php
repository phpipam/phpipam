...
class Subnets extends Common_functions
{
    ...
    public function subnet_split($subnet_old, $number, $prefix, $group = "yes", $strict = "yes", $copy_custom = "yes")
    {
        ...
        foreach ($newsubnets as $subnet) {
            ...
            if (sizeof($ids) > 0) {
                if (!$Admin->object_modify("ipaddresses", "edit-multiple", $ids, ["subnetId" => $this->lastInsertId])) {
        ...
        if ($group != "yes") {
            if (!$Admin->object_modify("subnets", "delete", "id", ["id" => $subnet_old->id])) {
    ...
    private function verify_subnet_split($subnet_old, $number, $group, $strict)
    {
        ...
        switch ($number) {
            ...
            //otherwise die
            default:
                $this->Result->show("danger", _("Invalid number of subnets"), true);
        }
        //set new mask
        ...
        return [0 => $newsubnets, 1 => $addresses];
    }
    ...
    public function modify_subnet($action, $values, $mail_changelog = true)
    {
        ...
        if ($action == "add") {
            ...
            return $this->subnet_edit($values, $mail_changelog);
    ...
    private function subnet_add($values)
    {
        ...
        # write changelog
        $this->Log->write_changelog('subnet', "add", 'success', [], $values);
        return true;
    }
    ...
    private function subnet_delete($id)
    {
        ...
        # write changelog
        $this->Log->write_changelog('subnet', "delete", 'success', $old_subnet, []);
        # ok
    ...
    private function create_subnet_familytree($subnetId)
    {
        ...
        try {
            ...
            if ($subnetId === false) {
                throw new Exception(_('Invalid subnetId'));
            }
        ...
        if (!empty($all_nats)) {
            ...
            foreach ($all_nats as $nat) {
                ...
                if (is_array($s['subnets']))
                    $s['subnets'] = array_diff($s['subnets'], [$obj_id]);
                if (is_array($d['subnets']))
                    $d['subnets'] = array_diff($d['subnets'], [$obj_id]);
    ...
    private function subnet_resize($subnetId, $subnet, $mask)
    {
        ...
        # execute
        try {
            $this->Database->updateObject("subnets", ["id" => $subnetId, "subnet" => $subnet, "mask" => $mask], "id");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
        ...
        # ok
        $this->Log->write("Subnet resize", "Subnet $old_subnet->description id $old_subnet->id resized<hr>" . $this->array_to_log(["id" => $subnetId, "mask" => $mask]), 0);
        return true;
    }
    ...
    public function fetch_all_subnets_search($type = "IPv4")
    {
        ...
        if ($type == "IPv4") {
            $query = "SELECT `id`,`subnet`,`mask` FROM `subnets` where CAST(`subnet` AS UNSIGNED) <= 4294967295;";
        } else {
            $query = "SELECT `id`,`subnet`,`mask` FROM `subnets` where CAST(`subnet` AS UNSIGNED) >  4294967295;";
        }
        # fetch
    ...
    {
        try {
            $subnets = $this->Database->getObjects("subnets");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    private function fetch_all_subnets_for_Check($discoverytype, $agentId)
    {
        ...
        try {
            $subnets = $this->Database->getObjectsQuery("SELECT `id`,`subnet`,`sectionId`,`mask`,`resolveDNS`,`nameserverId` FROM `subnets` WHERE `scanAgent` = ? AND `$discoverytype` = 1 AND COALESCE(`isFolder`,0) = 0 AND `mask` > 0;", [$agentId]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function is_linked($subnetId)
    {
        if (!is_numeric($subnetId)) {
            ...
            try {
                $subnets = $this->Database->getObjectsQuery("select * from subnets where `linked_subnet` = ?", [$subnetId]);
            } catch (Exception $e) {
                $this->Result->show("danger", _("Error: ") . $e->getMessage());
                return false;
            }
    ...
    public function fetch_scanned_subnets($agentId = null)
    {
        ...
        # fetch
        try {
            $subnets = $this->Database->getObjectsQuery($query, [$agentId]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function find_gateway($subnetId)
    {
        ...
        # fetch
        try {
            $gateway = $this->Database->getObjectQuery($query, [$subnetId]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function find_subnet($sectionId = false, $cidr = false)
    {
        ...
        // search
        try {
            $subnet = $this->Database->getObjectQuery("select * from `subnets` where `subnet`=? and `mask`=? and `sectionId`=?;", $values = [$this->transform_address($tmp[0], "decimal"), $tmp[1], $sectionId]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    private function calculate_single_subnet_details($subnet, $no_strict = false, $detailed = false)
    {
        ...
        if ($subnet->isFull == 1) {
            ...
            if ($ip_version == "IPv4" && !$strict_mode && $out["used"]) {
                if ($subnet->mask <= 30) {
                    $out["used"] = gmp_strval(gmp_add($out["used"], 2));
                }
            }
    ...
    private function calculate_subnet_usage_sort_addresses($subnet, $addresses = false, $strict_mode = true)
    {
        ...
        if ($strict_mode) {
            $ip_version = $this->get_ip_version($subnet->subnet);
            $subnet_broadcast = 2;
            if ($ip_version == "IPv4" && $subnet->mask >= 31) {
                $subnet_broadcast = 0;
            }
            if ($ip_version == "IPv6" && $subnet->mask >= 127) {
                $subnet_broadcast = 0;
            }
    ...
    public function fetch_section_subnets($sectionId, $field = false, $value = false, $result_fields = "*")
    {
        ...
        try {
            $subnets = $this->Database->getObjectsQuery($query, [$sectionId]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function find_inactive_hosts($timelimit = 86400, $limit = 100)
    {
        ...
        // search
        try {
            $res = $this->Database->getObjectsQuery("select ipaddresses.* from `ipaddresses` join subnets on ipaddresses.subnetId = subnets.id where subnets.pingSubnet = 1 and `lastSeen` between ? and ? order by lastSeen desc limit $limit;", [date("Y-m-d H:i:s", strtotime(date("Y-m-d H:i:s")) - $timelimit), date("Y-m-d H:i:s", strtotime(date("Y-m-d H:i:s")) - (int)str_replace(";", "", strstr($this->settings->pingStatus, ";")))]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function find_duplicate_multicast_mac($address_id, $mac)
    {
        ...
        // fetch
        try {
            $res = $this->Database->getObjectsQuery($query, [$mac, $address_id]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function set_permissions($subnets, $removed_permissions, $changed_permissions)
    {
        try {
            ...
            foreach ($subnets as $s) {
                ...
                // set values
                $values = ["id" => $s->id, "permissions" => json_encode($s_old_perm)];
    ...
    public function print_vlan_menu($user, $vlans, $section_subnets, $sectionId)
    {
        ...
        $html = [];
        # must be numeric
        if (isset($_GET['section'])) if (!is_numeric($_GET['section'])) {
            $this->Result->show("danger", _("Invalid ID"), true);
        }
        if (isset($_GET['subnetId'])) if (!is_numeric($_GET['subnetId'])) {
            $this->Result->show("danger", _("Invalid ID"), true);
        }
    ...
    public function print_mastersubnet_dropdown_menu($sectionId, $current_master = 0, $isFolder = false)
    {
        ...
        $folders = [];
        $section_subnets = $this->fetch_section_subnets($sectionId, false, false, ['id', 'masterSubnetId', 'isFolder', 'subnet', 'mask', 'description']);
    ...
    public function subnet_dropdown_print_available($sectionId, $subnetMasterId)
    {
        ...
        # must be integer
        if (isset($_GET['subnetId']) && !is_numeric($_GET['subnetId'])) {
            $this->Result->show("danger", _("Invalid ID"), true);
        }
        ...
        if (!is_object($parent) || $parent->isFolder == "1") {
            return "";
        };
    ...
    public function get_subnet_freespacemap($masterSubnet)
    {
        ...
        $subnets = is_array($subnets) ? $subnets : [];
        ...
        foreach ($subnets as $excl) {
            ...
            foreach ($ranges as $rid => $range) {
                if ((gmp_cmp($estart, $range['end']) > 0) || (gmp_cmp($eend, $range['start']) < 0)) {
                    continue;
                }
                
                # range overlaps, now we check what to do
                unset($ranges[$rid]); # remove existing range
                if (gmp_cmp($range['start'], $estart) < 0) {
                    $ranges[] = ['start' => $range['start'], 'end' => gmp_strval(gmp_sub($estart, 1))];
                };
                if (gmp_cmp($range['end'], $eend) > 0) {
                    $ranges[] = ['start' => gmp_strval(gmp_add($eend, 1)), 'end' => $range['end']];
                };
            }
        }
        uasort($ranges, function ($a, $b) {
            return gmp_cmp($a['start'], $b['start']);
        });
    ...
    public function get_freespacemap_first_available($fsm, $mask, $count)
    {
        if ($mask < 0 || $mask > $fsm['max_search_mask']) {
            return ['subnets' => [], 'truncated' => false];
        }
        ...
        foreach ($ranges as $range) {
            ...
            while (gmp_cmp($candidate_end, $range['end']) <= 0) {
                if ($count > 0 && ++$discovered > $count) {
                    return ['subnets' => $subnets, 'truncated' => true];
                }
        ...
        return ['subnets' => $subnets, 'truncated' => false];
    }
    ...
    public function search_available_subnets($subnetMasterId, $mask, $count = Subnets::SEARCH_FIND_ALL, $direction = Subnets::SEARCH_FIND_FIRST)
    {
        ...
        if (!is_object($parent) || $parent->isFolder == "1" || $parent->isFull == "1") {
            return false;
        };
        $usage = $this->calculate_subnet_usage($parent);
        if ($usage['freehosts'] == 0) {
            return false;
        }
        ...
        if (!is_numeric($mask) || $mask < 0 || $mask > $max_mask) {
            return false;
        }
    ...
    public function resolve_ripe_arin($subnet)
    {
        ...
        if (in_array($subnet_check, $this->ripe)) {
            ...
            return ["result" => "error", "error" => "$subnet Not RIPE or ARIN subnet"];
        }
    }
    ...
    private function query_ripe($subnet)
    {
        ...
        if ($ripe_result['result_code'] == 404) {
            // return array
            return ["result" => "error", "error" => $ripe_result['result']->errormessages->errormessage[0]->text];
        }
        // fail
        if ($ripe_result['result_code'] !== 200) {
            // return array
            return ["result" => "error", "error" => "Error connecting to ripe rest api : " . $ripe_result['error_msg']];
        } else {
            $out = [];
            // loop
            ...
            // return array
            return ["result" => "success", "data" => array_filter($out)];
        }
    }
    ...
    private function query_arin($subnet)
    {
        ...
        if ($arin_result['result_code'] == 404) {
            // return array
            return ["result" => "error", "error" => "Subnet not found"];
        }
        // fail
        if ($arin_result['result_code'] !== 200) {
            // return array
            return ["result" => "error", "error" => "Error connecting to arin rest api : " . $ripe_result['error_msg']];
        } else {
            $out = [];
            // loop
            ...
            if (array_key_exists("cidrLength", $out) && array_key_exists("startAddress", $out)) {
                $out = array_merge(["CIDR" => $out['startAddress'] . "/" . $out['cidrLength']], $out);
                $out = array_merge(['NetRange' => $out['startAddress'] . " - " . $out['endAddress']], $out);
