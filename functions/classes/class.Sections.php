...
class Sections extends Common_functions
{
    ...
    public function modify_section($action, $values)
    {
        ...
        if ($action == "add") {
            return $this->section_add($values);
        } elseif ($action == "edit") {
            return $this->section_edit($values);
        } elseif ($action == "delete") {
            return $this->section_delete($values);
        } elseif ($action == "reorder") {
            return $this->section_reorder($values);
        } else {
    ...
    private function section_add($values)
    {
        ...
        # execute
        try {
            $this->Database->insertObject("sections", $values);
        } catch (Exception $e) {
    ...
    private function section_edit($values)
    {
        ...
        # execute
        try {
            $this->Database->updateObject("sections", $values, "id");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
    ...
    {
        try {
            $subsections = $this->Database->getObjectsQuery("SELECT * FROM `sections` where `masterSection` = ?;", [$sectionId]);
        } catch (Exception $e) {
    ...
    private function section_reorder($order)
    {
        ...
        foreach ($order as $key => $o) {
            # execute
            try {
                $this->Database->updateObject("sections", ["order" => $o, "id" => $key], "id");
            } catch (Exception $e) {
                $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
                return false;
            }
    ...
    public function fetch_section_vlans($sectionId)
    {
        ...
        # fetch
        try {
            $vlans = $this->Database->getObjectsQuery($query, [$sectionId]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function fetch_section_vrfs($sectionId)
    {
        ...
        # fetch
        try {
            $vrfs = $this->Database->getObjectsQuery($query, [$sectionId]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
