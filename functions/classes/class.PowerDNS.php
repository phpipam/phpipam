...
class PowerDNS extends Common_functions {
    ...
    public function count_domain_records ($domain_id) {
        ...
        // fetch
        try { $records = $this->Database_pdns->getObjectsQuery("records", $query, [$domain_id]); }
        catch (Exception $e) {
            $this->Result->show("danger", _("Error: ").$e->getMessage());
            return false;
        }
    ...
    public function count_domain_records_by_type ($domain_id, $type="PTR") {
        ...
        // fetch
        try { $records = $this->Database_pdns->getObjectsQuery("records", $query, [$domain_id, $type]); }
        catch (Exception $e) {
            $this->Result->show("danger", _("Error: ").$e->getMessage());
            return false;
        }
    ...
    public function search_domains_by_hostname_or_ip ($hostname, $ip) {
        ...
        // fetch
        try { $records = $this->Database_pdns->getObjectsQuery("records", $query, [$hostname, $ip]); }
        catch (Exception $e) {
            $this->Result->show("danger", _("Error: ").$e->getMessage());
            return false;
        }
    ...
    public function search_record_domain_type_name ($domain_id, $type, $name) {
        ...
        // fetch
        try { $records = $this->Database_pdns->getObjectQuery("records", $query, [$domain_id, $type, $name]); }
        catch (Exception $e) {
            $this->Result->show("danger", _("Error: ").$e->getMessage());
            return false;
        }
    ...
    public function pdns_remove_ip_and_hostname_records ($hostname, $ip) {
       ...
       // run
       try { $this->Database_pdns->runQuery($query, [$hostname, $ip]); }
       catch (Exception $e) {
           $this->Result->show("danger", _("Error: ").$e->getMessage());
           return false;
       }
    ...
    private function update_soa_serial ($domain_id, $serial = false) {
        ...
        $soa_serial = explode(" ", (string) $soa->content);
    ...
    public function create_default_records ($values, $checkOnly = false) {
        ...
        // content
        $soa   = [];
        $soa[] = array_shift(explode(";", (string) $values['ns']));
    ...
    public function update_all_records ($domain_id, $old_name, $name) {
        // execute
        try { $this->Database_pdns->runQuery("update `records` set `name` = replace(`name`, ?, ?) where where `domain_id` = ?;", [$old_name, $name, $domain_id]); }
        catch (Exception $e) {
            // write log
            $this->Result->show("danger", _("Error: ").$e->getMessage());
            return false;
    ...
    public function remove_all_records ($domain_id) {
        // execute
        try { $this->Database_pdns->runQuery("delete from `records` where `domain_id` = ?;", [$domain_id]); }
        catch (Exception $e) {
            // write log
    ...
    public function get_ip_ptr_name ($ip) {
        ...
        if ($this->identify_address ($ip)=="IPv4") {
            $prefix = ".in-addr.arpa";
            $zone = array_reverse(explode(".", (string) $ip));
        }
        else {
            // PEAR for IPv6
    ...
    public function record_exists ($domain_id, $name, $type, $content) {
        // execute
        try { $res = $this->Database_pdns->getObjectQuery("records", "select count(*) as `cnt` from `records` where `domain_id` = ? and `name`=? and `type`=? and `content`=?;", [$domain_id, $name, $type, $content]); }
        catch (Exception $e) {
            $this->Result->show("danger", _("Error: ").$e->getMessage());
            return false;
        }
