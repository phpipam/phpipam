...
class Addresses extends Common_functions
{
    ...
    {
        try {
            $address = $this->Database->getObjectQuery("SELECT * FROM `ipaddresses` where `ip_addr` = ? and `subnetId` = ? limit 1;", [$ip_addr, $subnetId]);
        } catch (Exception $e) {
    ...
    private function bulk_fetch_similar_addresses($address, $linked_field, $value)
    {
        ...
        try {
            ...
            $linked_subnet_addrs = $this->Database->getObjectsQuery($query, [$address->subnetId]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function modify_address($address, $mail_changelog = true)
    {
        ...
        if ($address['action'] == "add") {
            return $this->modify_address_add($address);
        }                            //create new address
        elseif ($address['action'] == "edit") {
            return $this->modify_address_edit($address);
        }                            //modify existing address
        elseif ($address['action'] == "delete") {
            return $this->modify_address_delete($address);
        }                            //delete address
        elseif ($address['action'] == "move") {
            return $this->modify_address_move($address);
        }                            //move to new subnet
        else {
    ...
    protected function modify_address_add($address)
    {
        ...
        if ($address['is_gateway'] == 1) {
            $this->remove_gateway($address['subnetId']);
        }
        
        # execute
        try {
            $this->Database->insertObject("ipaddresses", $insert);
        } catch (Exception $e) {
        ...
        $this->Log->write_changelog('ip_addr', "add", 'success', [], $address, $this->mail_changelog);
    ...
    {
        try {
            $this->Database->updateObject("ipaddresses", ["subnetId" => $subnetId, "is_gateway" => 0], "subnetId");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function ptr_modify($action, $address, $print_error = true)
    {
        ...
        if ($this->settings->enablePowerDNS != 1) {
            ...
            # execute based on action
            if ($action == "add") {
                return $this->ptr_add($address, $print_error);
            }                            //create new PTR
            elseif ($action == "edit") {
                return $this->ptr_edit($address, $print_error);
            }                        //modify existing PTR
            elseif ($action == "delete") {
                return $this->ptr_delete($address, $print_error);
            }                        //delete PTR
            else {
                return $this->Result->show("danger", _("Invalid PDNS action"), true);
            }
        }
    }
    ...
    public function pdns_validate_connection($die = true)
    {
        ...
        # check connection
        if ($this->PowerDNS->db_check() === false && $die) {
            $this->Result->show("danger", _("Cannot connect to powerDNS database"), true);
        }
        # get settings
        $this->get_settings();
    }
    ...
    private function ptr_unlink($address_id)
    {
        # execute
        try {
            $this->Database->updateObject("ipaddresses", ["id" => $address_id, "PTR" => 0]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
    ...
    private function pdns_fetch_domain($subnet_id)
    {
        ...
        if ($subnet === false) {
            $this->Result->show("danger", _("Invalid subnet Id"), true);
        }
    ...
    public function ptr_link($address_id, $ptr_id)
    {
        # execute
        try {
            $this->Database->updateObject("ipaddresses", ["id" => $address_id, "PTR" => $ptr_id]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
    ...
    public function ptr_edit($address, $print_error = true)
    {
        ...
        if ($this->validate_hostname($address->hostname) === false) {
            ...
            if ($this->ptr_exists($address->PTR) === true) {
                $this->ptr_delete($address, $print_error);
            } else {
                return false;
            }
        }
    ...
    public function fetch_address($method, $id)
    {
        ...
        if (isset($this->addresses[$id])) {
            ...
            try {
                $address = $this->Database->getObjectQuery("SELECT * FROM `ipaddresses` where `$method` = ? limit 1;", [$id]);
            } catch (Exception $e) {
                $this->Result->show("danger", _("Error: ") . $e->getMessage());
                return false;
            }
    ...
    protected function modify_address_delete($address)
    {
        ...
        if ($address['type'] == "series") {
            $field = "subnetId";
            $value = $address['subnetId'];
            $field2 = "ip_addr";
            $value2 = $this->transform_address($address['ip_addr'], "decimal");
        } else {
            $field = "id";
            $value = $address['id'];
            $field2 = null;
        ...
        $this->Log->write_changelog('ip_addr', "delete", 'success', (array)$address_old, [], $this->mail_changelog);
    ...
    public function remove_address_nat_items($obj_id = 0, $print = true)
    {
        ...
        # fetch all nats
        try {
            $all_nats = $this->Database->getObjectsQuery("select * from `nat` where `src` like :id or `dst` like :id", ["id" => '%"' . $obj_id . '"%']);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        ...
        if (!empty($all_nats)) {
            ...
            foreach ($all_nats as $nat) {
                ...
                if (is_array($s['ipaddresses']))
                    $s['ipaddresses'] = array_diff($s['ipaddresses'], [$obj_id]);
                if (is_array($d['ipaddresses']))
                    $d['ipaddresses'] = array_diff($d['ipaddresses'], [$obj_id]);
                ...
                if ($s != $src_new || $d != $dst_new) {
                    ...
                    if ($Admin->object_modify("nat", "edit", "id", ["id" => $nat->id, "src" => $src_new, "dst" => $dst_new]) !== false) {
                        if ($print) {
    ...
    protected function modify_address_move($address)
    {
        # execute
        try {
            $this->Database->updateObject("ipaddresses", ["subnetId" => $address['newSubnet'], "id" => $address['id']]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
    ...
    public function update_address_hostname($ip, $id, $hostname = "")
    {
        if (is_numeric($id) && strlen($hostname) > 0) {
            try {
                $this->Database->updateObject("ipaddresses", ["id" => $id, "hostname" => $hostname]);
            } catch (Exception $e) {
                return false;
            }
            // save log
            $this->Log->write("Address DNS resolved", "Address $ip resolved<hr>" . $this->array_to_log((array)$hostname), 0);
            $this->Log->write_changelog('ip_addr', "edit", 'success', ["id" => $id, "hostname" => ""], ["id" => $id, "hostname" => $hostname], $this->mail_changelog);
        }
    }
    ...
    public function get_first_available_address($subnetId, $Subnets)
    {
        ...
        $addresses = $this->fetch_subnet_addresses($subnetId, "ip_addr", "asc", ["ip_addr"]);
        ...
        # false if slaves
        $this->Subnets = new Subnets ($this->Database);
        if ($this->Subnets->has_slaves($subnetId)) {
            return false;
        }
...
//        $max_hosts = $Subnets->get_max_hosts($subnet['mask'], $this->identify_address($subnet['subnet']));
        ...
        # full subnet?
        if (sizeof($addresses) >= $max_hosts || $subnet['isFull']) {
            return false;
        }    //full subnet
    ...
    public function fetch_subnet_addresses($subnetId, $order = null, $order_direction = null, $fields = "*")
    {
        ...
        try {
            $addresses = $this->Database->getObjectsQuery("SELECT $fields FROM `ipaddresses` where `subnetId` = ? order by `$order[0]` $order[1];", [$subnetId]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        ...
        # result
        return sizeof($addresses) > 0 ? $addresses : [];
    }
    ...
    {
        try {
            $this->Database->runQuery("update `ipaddresses` set `PTR` = 0 where `subnetId` = ?;", [$subnet_id]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    {
        try {
            $indexes = $this->Database->getObjectsQuery("select `PTR` from `ipaddresses` where `PTR` != 0 and `subnetId` = ?;", [$subnetId]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function import_address_from_csv($address, $subnetId)
    {
        ...
        # verify address
        if ($this->verify_address($address[0], $this->transform_to_dotted($subnet['subnet']) . "/" . $subnet['mask'], false, false) !== false) {
            return false;
        }
        # check for duplicates
        if ($this->address_exists($address[0], $subnetId)) {
            return _('IP address already exists') . ' - ' . $address[0];
        }
        
        # format insert array
        $address_insert = ["subnetId" => $subnetId,
            "ip_addr" => $address[0],
            "state" => $address[1],
    ...
    public function verify_address($address, $subnet, $no_strict = false, $die = true)
    {
        ...
        $this->initialize_subnets_object();
        if (strlen($error = $this->Subnets->verify_cidr($subnet)) > 1) {
            $this->Result->show("danger", $error, $die);
            return true;
        }
    ...
    public function verify_address_IPv6($address, $subnet, $die)
    {
        ...
        elseif (!$this->Net_IPv6->isInNetmask($address, $subnet)) {
    ...
    public function verify_address_IPv4($address, $subnet, $no_strict, $die)
    {
        ...
        # is address valid?
        if (!$this->Net_IPv4->validateIP($address)) {
            $this->Result->show("danger", _("IP address not valid") . "! ($address)", $die);
            return true;
        } # is address in provided subnet
        elseif (!$this->Net_IPv4->ipInNetwork($address, $subnet)) {
            $this->Result->show("danger", _("IP address not in selected subnet") . "! ($address)", $die);
            return true;
        ...
        elseif ($mask[1] == 31 || $mask[1] == 32 || $no_strict == true) {
            ...
            if ($net->network == $address) {
                $this->Result->show("danger", _("Cannot add subnet as IP address!"), $die);
                return true;
            } elseif ($net->broadcast == $address) {
                $this->Result->show("danger", _("Cannot add broadcast as IP address!"), $die);
                return true;
            }
        }
        # default
    ...
    public function address_exists($address, $subnetId, $cnt = true)
    {
        ...
        if ($cnt === true) {
            ...
            $query = "select `id` from `ipaddresses` where `subnetId`=? and `ip_addr`=?;";
        }
        # fetch
        try {
            $count = $this->Database->getObjectQuery($query, [$subnetId, $address]);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        ...
        if ($cnt === true) {
            ...
            return is_null($count->id) ? false : $count->id;
        }
    }
    ...
    {
        try {
            $count = $this->Database->numObjectsFilter("ipaddresses", "subnetId", $subnetId);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function fetch_subnet_addresses_recursive($subnetId, $count = false, $order = null, $order_direction = null)
    {
        ...
        # ip address order
        if (!is_null($order)) {
            $order_addr = [$order, $order_direction];
        } else {
            $order_addr = ["ip_addr", "asc"];
        }
        ...
        if ($count) {
            $query = 'select count(*) as cnt from `ipaddresses` where `subnetId` = ? ';
        } else {
        ...
        # fetch
        try {
            $addresses = $this->Database->getObjectsQuery($query, $ids);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function find_unused_addresses($address1, $address2, $netmask, $empty = false, $is_subnet = false, $is_broadcast = false, $no_strict = false)
    {
        ...
        # check for space
        return $this->identify_address($address1) == "IPv6" ? $this->find_unused_addresses_IPv6($address1, $address2, $netmask, $empty, $is_subnet, $is_broadcast, $no_strict) : $this->find_unused_addresses_IPv4($address1, $address2, $netmask, $empty, !$no_strict);
    }
     ...
     * @return void
     */
    protected function find_unused_addresses_IPv6($address1, $address2, $netmask, $empty = false, $is_subnet = false, $is_broadcast = false, $no_strict = false)
    {
        # Initialize PEAR NET object
        $this->initialize_pear_net_IPv6();
        ...
        if ($empty) {
            ...
            return ["ip" => $this->transform_to_dotted(gmp_strval($address1)) . " - " . $this->transform_to_dotted(gmp_strval($address2)), "hosts" => $Subnets->get_max_hosts($netmask, "IPv6")];
        } else {
            # calculate diff
            ...
            if ($netmask == 128) {
                if ($diff > 1) {
                    return ["ip" => $this->transform_to_dotted(gmp_strval($address1)), "hosts" => 1];
                }
            } # /127
            elseif ($netmask == 127) {
                if ($diff == 1 && $this->is_network($address1, $netmask)) {
                    return ["ip" => $this->transform_to_dotted($address2), "hosts" => 1];
                } elseif ($diff == 1 && $this->is_broadcast($address2, $netmask)) {
                    return ["ip" => $this->transform_to_dotted($address1), "hosts" => 1];
                } elseif ($diff == 0) {
                    return false;
                } else {
                    return ["ip" => $this->transform_to_dotted($address1), "hosts" => 2];
                }
            } # null
            elseif ($diff == 0) {
                return false;
            ...
            elseif ($diff == 1) {
                if ($is_subnet) {
                    ...
                    return ["ip" => $this->transform_to_dotted(gmp_strval($address2)), "hosts" => 1];
                } else {
                    return false;
                }
            } # diff 2
            elseif ($diff == 2 && !$is_subnet && !$is_broadcast) {
                return ["ip" => $this->transform_to_dotted(gmp_strval(gmp_add($address1, 1))), "hosts" => 1];
            } # default
            else {
                if ($is_subnet) {
                    return ["ip" => $this->transform_to_dotted(gmp_strval($address1)) . " - " . $this->transform_to_dotted(gmp_strval(gmp_sub($address2, 1))), "hosts" => $this->reformat_number(gmp_strval(gmp_sub($diff, 0)))];
                } elseif ($is_broadcast) {
                    return ["ip" => $this->transform_to_dotted(gmp_strval(gmp_add($address1, 1))) . " - " . $this->transform_to_dotted(gmp_strval($address2)), "hosts" => $this->reformat_number(gmp_strval(gmp_sub($diff, 0)))];
                } else {
                    return ["ip" => $this->transform_to_dotted(gmp_strval(gmp_add($address1, 1))) . " - " . $this->transform_to_dotted(gmp_strval(gmp_sub($address2, 1))), "hosts" => $this->reformat_number(gmp_strval(gmp_strval(gmp_sub($diff, 1))))];
                }
            }
    ...
    protected function find_unused_addresses_IPv4($address1, $address2, $netmask, $empty, $strict = true)
    {
        ...
        elseif ($diff == 2) {
            return ["ip" => $this->transform_to_dotted($address1 + 1), "hosts" => 1];
        } # if diff is more than 2 return pool */
        else {
            if($strict == true) {
                return ["ip"=>$this->transform_to_dotted($address1)." - ".$this->transform_to_dotted(($address2)), "hosts"=>gmp_strval(gmp_sub($diff+2, 1))];
            }
            return ["ip" => $this->transform_to_dotted($address1 + 1) . " - " . $this->transform_to_dotted(($address2 - 1)), "hosts" => gmp_strval(gmp_sub($diff, 1))];
        }
        # default false
        return false;
    }
    ...
    {
        try {
            $cnt = $this->Database->numObjectsFilter("ipaddresses", "hostname", $hostname);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    {
        try {
            $res = $this->Database->getObjectsQuery("select distinct(`subnetId`) from `ipaddresses` order by `subnetId` asc;");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    {
        try {
            $res = $this->Database->numObjectsFilter("subnets", "id", $id);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
