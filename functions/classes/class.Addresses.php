...
class Addresses extends Common_functions
{
    ...
    public function fetch_address($method, $id)
    {
        ...
        if (isset($this->addresses[$id])) {
            ...
            try { $address = $this->Database->getObjectQuery("SELECT * FROM `ipaddresses` where `$method` = ? limit 1;", array($id));} catch (Exception $e) {
    ...
    {
        try { $address = $this->Database->getObjectQuery("SELECT * FROM `ipaddresses` where `ip_addr` = ? and `subnetId` = ? limit 1;", array($ip_addr, $subnetId));} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        # save to addresses cache
    ...
    public function search_similar_addresses($linked_field, $value, $address_id)
    {
        ...
        if (strlen($linked_field) > 0 && strlen($value) > 0 && is_numeric($address_id)) {
            // search
            try { $addresses = $this->Database->getObjectsQuery("SELECT * FROM `ipaddresses` where `$linked_field` = ? and `id` != ? and state != 4;", array($value, $address_id));} catch (Exception $e) {
                $this->Result->show("danger", _("Error: ") . $e->getMessage());
                return false;
            }
            #result
    ...
    protected function modify_address_add($address)
    {
        ...
        # execute
        try { $this->Database->insertObject("ipaddresses", $insert);} catch (Exception $e) {
    ...
    protected function modify_address_edit($address)
    {
        ...
        # execute
        try { $this->Database->updateObject("ipaddresses", $insert, $id1, $id2);} catch (Exception $e) {
    ...
    protected function modify_address_delete($address)
    {
        ...
        # execute
        try { $this->Database->deleteRow("ipaddresses", $field, $value, $field2, $value2);} catch (Exception $e) {
    ...
    {
        # execute
        try { $this->Database->updateObject("ipaddresses", array("subnetId" => $address['newSubnet'], "id" => $address['id']));} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
        # ok
    ...
    {
        try { $this->Database->updateObject("ipaddresses", array("subnetId" => $subnetId, "is_gateway" => 0), "subnetId");} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    }
    ...
    public function address_exists($address, $subnetId, $cnt = true)
    {
        ...
        # check
        if ($cnt === true) {$query = "select count(*) as `cnt` from `ipaddresses` where `subnetId`=? and `ip_addr`=?;";} else { $query = "select `id` from `ipaddresses` where `subnetId`=? and `ip_addr`=?;";}
        # fetch
        try { $count = $this->Database->getObjectQuery($query, array($subnetId, $address));} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        # result
        if ($cnt === true) {return $count->cnt == 0 ? false : true;} else {return is_null($count->id) ? false : $count->id;}
    }
    ...
    public function get_first_available_address($subnetId, $Subnets)
    {
            ...
            foreach ($addresses as $k => $ipaddress) {
                ...
                if ($k == 0) {
                    ...
                    if ($subnet['mask'] == 31) {
                        ...
                        if (gmp_strval(gmp_sub($addresses[$k]->ip_addr, $subnet['subnet'])) > 1) {return gmp_strval(gmp_add($subnet['subnet'], 1));} elseif ($ip_version == "IPv6") {
        ...
        # no addresses
            ...
            # /32, /31
            if ($subnet['mask'] == 32 || $subnet['mask'] == 31 || $ip_version == "IPv6") {return $subnet['subnet'];} else {return gmp_strval(gmp_add($subnet['subnet'], 1));}
        }
    ...
    public function ptr_edit($address, $print_error = true)
    {
        ...
        if ($this->validate_hostname($address->dns_name) === false) {
            ...
            if ($this->ptr_exists($address->PTR) === true) {$this->ptr_delete($address, $print_error);} else {return false;}
        }
        ...
        // update PTR
            ...
            // create insert array
            $update = $this->PowerDNS->formulate_update_record($domain->id, $this->PowerDNS->get_ip_ptr_name($this->transform_address($address->ip_addr, "dotted")), null, $address->dns_name, null, null, null, $old_record->change_date);
    ...
    {
        # execute
        try { $this->Database->updateObject("ipaddresses", array("id" => $address_id, "PTR" => $ptr_id));} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
    }
    ...
    {
        # execute
        try { $this->Database->updateObject("ipaddresses", array("id" => $address_id, "PTR" => 0));} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
            return false;
        }
    }
    ...
    {
        try { $this->Database->runQuery("update `ipaddresses` set `PTR` = 0 where `subnetId` = ?;", array($subnet_id));} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        #result
    ...
    {
        try { $indexes = $this->Database->getObjectsQuery("select `PTR` from `ipaddresses` where `PTR` != 0 and `subnetId` = ?;", array($subnetId));} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        # parse
    ...
    {
        # set order
        if (!is_null($order)) {$order = array($order, $order_direction);} else { $order = array("ip_addr", "asc");}

        # escape ordering
        $order[0] = $this->Database->escape($order[0]);
        $order[1] = $this->Database->escape($order[1]);

        try { $addresses = $this->Database->getObjectsQuery("SELECT * FROM `ipaddresses` where `subnetId` = ? order by `$order[0]` $order[1];", array($subnetId));} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        # save to addresses cache
    ...
    {
        try { $count = $this->Database->numObjectsFilter("ipaddresses", "subnetId", $subnetId);} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        # result
    ...
    public function count_addresses_in_multiple_subnets($subnets)
    {
        ...
        foreach ($subnets as $k => $s) {
            if (is_object($s)) {$tmp[] = " `subnetId`=$s->id ";} else { $tmp[] = " `subnetId`=$s ";}
        }
        $query = "select count(*) as `cnt` from `ipaddresses` where " . implode("or", $tmp) . ";";

        # fetch
        try { $addresses = $this->Database->getObjectsQuery($query);} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        # return count
    ...
    public function fetch_subnet_addresses_recursive($subnetId, $count = false, $order = null, $order_direction = null)
    {
        ...
        # ip address order
        if (!is_null($order)) {$order_addr = array($order, $order_direction);} else { $order_addr = array("ip_addr", "asc");}
        ...
        if ($count) {$query = 'select count(*) as cnt from `ipaddresses` where `subnetId` = ? ';} else { $query = 'select * from `ipaddresses` where `subnetId` = ? ';}
        ...
        # fetch
        try { $addresses = $this->Database->getObjectsQuery($query, $ids);} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function verify_address_IPv4($address, $subnet, $no_strict, $die)
    {
        ...
        # It cannot be subnet or broadcast
            ...
            if ($net->network == $address) {$this->Result->show("danger", _("Cannot add subnet as IP address!"), $die);return true;} elseif ($net->broadcast == $address) {$this->Result->show("danger", _("Cannot add broadcast as IP address!"), $die);return true;}
        }
        # default
        return false;
    }
    ...
    {
        try { $cnt = $this->Database->numObjectsFilter("ipaddresses", "dns_name", $hostname);} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    {
        try { $res = $this->Database->getObjectsQuery("select distinct(`subnetId`) from `ipaddresses` order by `subnetId` asc;");} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        # return
    ...
    {
        try { $res = $this->Database->getObjectQuery("select count(*) as `cnt` from `subnets` where `id` = ?;", array($id));} catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        # return
